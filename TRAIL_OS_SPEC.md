...
> Version: 1.0 · Phase 1 Complete · June 2026  
> Status: Internal prototype — no live user authentication, no public deployment yet
...
Trail OS has four user tiers. The active tier is set via the tier switcher in the sidebar footer during the prototype phase. In production, tier will be assigned from the user's Salesforce profile.
...
| Live integrations | Salesforce REST + Slack POC | ✅ Live |
| Google OAuth | In progress | ⏳ Tokens needed |
| Automated test suite | 0 | ❌ Phase 2 |
| Metadata-driven readiness cases | 70 | ✅ |
| Hardcoded content items | 26 | Classified (12 OK, 11 Phase 2, 3 stale) |
...
### Ask Penny (Phase 1 POC)

Ask Penny in the right panel is a Phase 1 POC — the UI is present but responses are either hardcoded or powered by a Gemini POC connection tested at `/penny/test`. The full conversational flow (RAG, memory, tool use) is Phase 2.
...
**Path**: `/penny`  
**Min tier**: Admin  
**Pattern**: Multi-tab hub

| Tab | Path | Content |
|---|---|---|
| Capabilities | `/penny` | Capability workspace with governance tabs |
...
| Test Penny | `/penny/test` | Live Gemini POC test interface |

The Penny hub also includes tier-gated POC integration tabs for the Phase 1 integration layer (`/penny/integration-layer`).
...
| `/admin/people-access` | People & Access | Admin | Role assignment and access management |
...
| State | Meaning |
|---|---|
| `live` | Connected and operating (Salesforce REST, Slack POC) |
| `partial` | Connected but incomplete (Google OAuth — tokens pending) |
| `planned` | Architecture defined, not yet connected (Gemini, Agentforce) |
| `future` | Roadmap item, no architecture yet |
...
- **Status**: OAuth in progress  
- **Flow**: 5-endpoint route at `/api/google/oauth/*` — initiate, callback, token exchange, refresh, revoke  
- **Wizard**: `/admin/google-oauth` — 5-step UI wizard  
- **Blocker**: #1 issue is Google OAuth app in "Testing" mode — all test users must be explicitly added in Google Cloud Console  
- **Scope**: Drive + Calendar + Docs + Sheets + Mail integrations planned  
- **Note**: `req.params` needs `as string` cast in Express 5 types

### Gemini

- **Status**: Phase 2 planned  
- **POC**: `/penny/test` provides a live Gemini test interface (POC only, not used in production flows)  
- **API route**: `POST /api/gemini/*`  
- **Phase 2 plan**: Full Penny LLM integration — RAG over knowledge sources, contextual coaching responses, Trail Quest delivery

### Agentforce

- **Status**: Phase 2 planned  
- **Relationship**: Coexistence model — Penny (Trail OS AI layer) + Agentforce (Salesforce AI layer) will operate as complementary agents, not duplicates  
- **Handoff protocol**: To be defined in Phase 2
...
- `googleCalendarData.ts` — calendar events (to be replaced by live Calendar API)
