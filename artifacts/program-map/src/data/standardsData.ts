...
export const contentStandards: ContentStandard[] = [
...

  {
    ...
    requiredFields: [
      ...
      { field: 'Salesforce Object Map',    description: 'Which SF objects power this program (Program__c, Service_Schedule__c, Training_Plan__c, etc.).',          required: true },
    ...
    exampleOutput: 'Reference Implementation — Foundations Trail: 12-week cohort, 4 sprints, 12 modules. Target: SF Admin + Associate certifications. Salesforce: Program__c, Service_Schedule__c, Program_Engagement__c, Training_Plan__c. Drive: Foundations Trail — Program Workspace (Active). Penny: 12 coaching prompts, 12 reflection prompts, 4 trail quests, 4 weekly reviews. Calendar: 48 sessions + 12 office hours. This shows the blueprint applied to a real program — not the blueprint itself.',
    sfMapping: 'Program__c (PMM) — top-level program record. Service_Schedule__c for cohort scheduling. Training_Plan__c links program to LMS modules.',
    lmsMapping: 'Program-level record in LMS; maps to course catalog entry. All modules are course units with Training_Plan_Item__c assessment links.',
