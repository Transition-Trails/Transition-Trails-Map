...
export const CAL_VALIDATION_CHECKS: CalValidationCheck[] = [
  { id:'cv-01', category:'Credentials', label:'GOOGLE_CLIENT_ID configured',          status:'fail',    detail:'GOOGLE_CLIENT_ID not set. Shared with Drive integration.',             impact:'Cannot initiate OAuth 2.0 flow for Google Calendar access.',                fix:'Set GOOGLE_CLIENT_ID from Google Cloud Console → Credentials.' },
  { id:'cv-02', category:'Credentials', label:'GOOGLE_CLIENT_SECRET configured',      status:'fail',    detail:'GOOGLE_CLIENT_SECRET not set. Shared with Drive integration.',         impact:'OAuth 2.0 authentication will fail. No Calendar API access possible.',      fix:'Set GOOGLE_CLIENT_SECRET from Google Cloud Console → Credentials.' },
  { id:'cv-03', category:'Credentials', label:'GOOGLE_CALENDAR_REFRESH_TOKEN set',    status:'fail',    detail:'Calendar-specific refresh token not configured.',                       impact:'Calendar sessions will expire and cannot be refreshed.',                    fix:'Complete Calendar OAuth flow and store in GOOGLE_CALENDAR_REFRESH_TOKEN.' },
...
export const CAL_GOVERNANCE_ISSUES: CalGovernanceIssue[] = [
  ...
  { id:'cgi-02', severity:'Critical', category:'Permission',    title:'Google Calendar credentials not configured',       detail:'GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_CALENDAR_REFRESH_TOKEN are all missing. No calendar access possible.',            affectedObjects:['All Calendars', 'All Events'],                                             resolution:'Configure Google Calendar OAuth credentials in Replit environment secrets.', status:'Open' },
...
export const CAL_TEST_SUITES: CalTestSuite[] = [
  {
    ...
    tests: [
      { id:'ct-01', name:'GOOGLE_CLIENT_ID present',         description:'Verify GOOGLE_CLIENT_ID is set in environment.', status:'fail',    result:'Secret not found. Set GOOGLE_CLIENT_ID.' },
      { id:'ct-02', name:'GOOGLE_CLIENT_SECRET present',     description:'Verify GOOGLE_CLIENT_SECRET is set.',            status:'fail',    result:'Secret not found. Set GOOGLE_CLIENT_SECRET.' },
      { id:'ct-03', name:'GOOGLE_CALENDAR_REFRESH_TOKEN set',description:'Calendar refresh token configured.',              status:'fail',    result:'Refresh token not found. Complete Calendar OAuth flow.' },
