...
export const CAL_VALIDATION_CHECKS: CalValidationCheck[] = [
  ...
  { id:'cv-03', category:'Credentials', label:'GOOGLE_CALENDAR_REFRESH_TOKEN set',    status:'pass',    detail:'GOOGLE_CALENDAR_REFRESH_TOKEN obtained Jun 13 2026 via Google OAuth flow — stored in Replit Secrets. Calendar API confirmed live: /api/calendar/events returns 10 real events from primary calendar.', impact:'Calendar API sessions can be established.', fix:'' },
  { id:'cv-04', category:'OAuth',       label:'Calendar API enabled in Google Cloud', status:'pass',    detail:'Calendar API confirmed active — live event fetch at /api/calendar/events returned HTTP 200 with real event data (Jun 13 2026).',          impact:'OAuth + API both operational.',           fix:'' },
  { id:'cv-05', category:'Scopes',      label:'calendar.readonly scope declared',     status:'pass',    detail:'calendar.readonly scope active — event listing with attendee metadata confirmed working via /api/calendar/events.',       impact:'Events and attendees listing operational.',         fix:'' },
  { id:'cv-06', category:'Scopes',      label:'calendar.events scope declared',       status:'warning', detail:'calendar.events write scope not yet verified. Penny-created reminders and pre-brief events require this scope.', impact:'Penny cannot yet create reminders or calendar items.',      fix:'Test calendar.events write scope when Penny scheduling is wired in Phase 2.' },
  { id:'cv-07', category:'Scopes',      label:'calendar.settings.readonly scope',     status:'warning', detail:'Settings scope not yet verified. Timezone read operations unconfirmed.',           impact:'Cannot confirm workspace timezone config from API.',         fix:'Add calendar.settings.readonly scope when timezone enforcement is implemented.' },
  { id:'cv-08', category:'Calendar Access', label:'Program calendars accessible',     status:'warning', detail:'Primary calendar confirmed accessible. Program-specific calendars (FT, GT, ET) are not yet mapped to Google Calendar IDs.',                    impact:'Calendar Registry not yet live-synced from program calendars.',         fix:'Share program calendars with the Google account used for OAuth and map their IDs.' },
  { id:'cv-09', category:'Calendar Access', label:'Operations calendar accessible',   status:'warning', detail:'Primary calendar access is live. Operations calendar not yet mapped.',                              impact:'Operations scheduling layer not yet connected.',                     fix:'Share operations calendar and map Google Calendar ID.' },
  { id:'cv-10', category:'Calendar Access', label:'Executive calendar accessible',    status:'warning', detail:'Primary calendar access confirmed. Executive calendar not yet mapped — may have restricted visibility.',                    impact:'Executive review scheduling not yet available.',                       fix:'Executive Lead to share calendar with OAuth account and map ID.' },
  { id:'cv-11', category:'Permissions', label:'Event read permissions',               status:'pass',    detail:'Event read confirmed — /api/calendar/events returns events with summary, start, end, attendees, htmlLink, and responseStatus.',                  impact:'Event data readable from primary calendar.',                    fix:'' },
  { id:'cv-12', category:'Permissions', label:'Event write permissions (Penny)',      status:'warning', detail:'Event write scope not yet verified. Required for Penny scheduling capabilities (pre-brief reminders, attendance nudges).',      impact:'Penny scheduling write capabilities blocked until scope confirmed.',                            fix:'Test calendar.events write scope in Phase 2 Penny scheduling sprint.' },
];
...
export const PENNY_SCHEDULING_CAPABILITIES: PennySchedulingCapability[] = [
  {
    ...
    status:'Planned',
    readiness:'Partial',
    blockReason:'Program-specific Google Calendar IDs not yet mapped. Credentials are live — primary calendar works. Map FT program/cohort calendars to enable sprint-aligned triggers.',
  ...
  },
  {
    ...
    status:'Planned',
    readiness:'Not Ready',
    blockReason:'Credentials live — primary calendar accessible. Post-event trigger needs program calendar IDs mapped and calendar.events write scope confirmed.',
  ...
  },
  {
    ...
    status:'Planned',
    readiness:'Not Ready',
    blockReason:'Credentials live — primary calendar events readable. Program-specific calendar IDs not yet mapped; attendance nudges need cohort calendar access.',
...
export const CAL_GOVERNANCE_ISSUES: CalGovernanceIssue[] = [
  ...
  { id:'cgi-02', severity:'High',     category:'Permission',    title:'Google Calendar refresh token — resolved',            detail:'GOOGLE_CALENDAR_REFRESH_TOKEN obtained Jun 13 2026 via OAuth flow. Live calendar API now active — /api/calendar/events returns real events from primary calendar. Calendar Action Panel live at /collaboration/calendar-live.',         affectedObjects:['Primary Calendar'],                                             resolution:'Resolved Jun 13 2026. Next step: map program-specific Google Calendar IDs.', status:'Resolved' },
...
export const CAL_TEST_SUITES: CalTestSuite[] = [
  {
    ...
    tests: [
      ...
      { id:'ct-03', name:'GOOGLE_CALENDAR_REFRESH_TOKEN set',description:'Calendar refresh token configured.',              status:'pass',    result:'GOOGLE_CALENDAR_REFRESH_TOKEN set Jun 13 2026. Live event fetch confirms token is valid — 10 events returned from primary calendar.' },
      { id:'ct-04', name:'Calendar API enabled in GCP',      description:'Google Calendar API enabled in project.',        status:'pass',    result:'API confirmed enabled — /api/calendar/events returned HTTP 200 with real data Jun 13 2026.' },
      { id:'ct-05', name:'calendar.readonly scope active',   description:'Read scope declared and granted.',               status:'pass',    result:'calendar.readonly scope confirmed — event listing with attendee metadata (responseStatus, displayName) working.' },
      { id:'ct-06', name:'calendar.events scope active',     description:'Events write scope for Penny creation.',         status:'warning', result:'Write scope not yet tested. Required for Phase 2 Penny scheduling (reminders, pre-brief creation).' },
    ],
  },
  {
    ...
    tests: [
      { id:'ct-07', name:'FT Program Calendar accessible',    description:'Can read Foundations Trail Program Calendar.', status:'warning', result:'Credentials live — primary calendar verified. FT Program Calendar not yet mapped (googleCalendarId undefined). Map Google Calendar ID to unblock.' },
      { id:'ct-08', name:'FT Cohort 2 Calendar accessible',   description:'Can read FT Cohort 2 Calendar.',              status:'warning', result:'Credentials live — primary calendar verified. FT Cohort 2 Calendar not yet mapped. Map Google Calendar ID to unblock.' },
      { id:'ct-09', name:'Office Hours Calendar accessible',  description:'Can read Office Hours Calendar.',             status:'warning', result:'Credentials live. Office Hours Calendar not yet shared with OAuth account or mapped.' },
      { id:'ct-10', name:'Coaching Calendar accessible',      description:'Can read Coaching Sessions Calendar.',        status:'warning', result:'Credentials live. Coaching Calendar not yet shared with OAuth account or mapped.' },
      { id:'ct-11', name:'Executive Calendar accessible',     description:'Can read Executive Reviews Calendar.',        status:'warning', result:'Credentials live. Executive Reviews Calendar not yet shared with OAuth account or mapped.' },
    ],
  },
  {
...
export const CAL_HEALTH_SCORES: CalHealthScore[] = [
  { dimension:'credentials', label:'Account & Credentials', score:10, maxScore:10, status:'ready', note:'All credentials live — OAuth token obtained Jun 13 2026. Live API confirmed.', items:[{ label:'GOOGLE_CLIENT_ID', status:'pass', note:'Set and verified' },{ label:'GOOGLE_CLIENT_SECRET', status:'pass', note:'Set and verified' },{ label:'Calendar Refresh Token', status:'pass', note:'Obtained Jun 13 2026 · live event fetch confirmed' }] },
  { dimension:'calendar-access', label:'Calendar Access', score:4, maxScore:10, status:'partial', note:'Primary calendar live; program-specific calendars need Google Calendar ID mapping', items:[{ label:'Primary Calendar', status:'pass', note:'Live — 10 events returned' },{ label:'FT Program Calendar', status:'partial', note:'Defined, Google ID not yet mapped' },{ label:'Cohort Calendars', status:'partial', note:'2 defined, IDs not mapped' },{ label:'Operations Calendar', status:'fail', note:'Not mapped' },{ label:'Executive Calendar', status:'fail', note:'Not mapped' }] },
  { dimension:'event-readiness', label:'Event Readiness', score:7, maxScore:10, status:'partial', note:'15 catalogued + live primary calendar events; program calendars not yet mapped', items:[{ label:'Events Catalogued', status:'pass', note:'15 events across 9 types' },{ label:'Live Primary Calendar', status:'pass', note:'Real events via /api/calendar/events' },{ label:'Program Mappings', status:'pass', note:'All events have program links' },{ label:'Timezone Consistency', status:'partial', note:'Not enforced at API level' }] },
  ...
  { dimension:'penny-readiness', label:'Penny Scheduling Readiness', score:5, maxScore:10, status:'partial', note:'Credentials live; Calendar Action Panel active; write scope and program calendars still needed', items:[{ label:'Calendar Action Panel', status:'pass', note:'Live at /collaboration/calendar-live — real events + Penny prep brief' },{ label:'Weekly Reflection Trigger', status:'partial', note:'Prototype via Slack schedule' },{ label:'Pre-Session Brief', status:'partial', note:'Credentials live; program calendar IDs needed' },{ label:'Attendance Nudge', status:'partial', note:'Credentials live; write scope needed' },{ label:'Escalation Follow-up', status:'partial', note:'Partially defined' }] },
  ...
  { dimension:'governance', label:'Governance Readiness', score:5, maxScore:10, status:'partial', note:'Refresh token issue resolved; 1 calendar ownership and sprint coverage issues remain open', items:[{ label:'Calendar Ownership', status:'partial', note:'1 of 11 calendars missing owner' },{ label:'OAuth Token', status:'pass', note:'Resolved Jun 13 2026' },{ label:'Timezone Enforcement', status:'partial', note:'Not enforced at API level' },{ label:'Event Completeness', status:'partial', note:'Sprint 4+ events missing' },{ label:'Duplicate Prevention', status:'pass', note:'No duplicates detected' }] },
];
