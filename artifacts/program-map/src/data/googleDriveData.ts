...
export const DRIVE_VALIDATION_CHECKS: DriveValidationCheck[] = [
  { id:'dv-01', category:'Credentials', label:'GOOGLE_CLIENT_ID configured',        status:'fail',    detail:'GOOGLE_CLIENT_ID not set in environment secrets.',              impact:'Cannot initiate OAuth 2.0 flow for Google Drive access.',          fix:'Set GOOGLE_CLIENT_ID from Google Cloud Console → APIs & Services → Credentials.' },
  { id:'dv-02', category:'Credentials', label:'GOOGLE_CLIENT_SECRET configured',    status:'fail',    detail:'GOOGLE_CLIENT_SECRET not set in environment secrets.',          impact:'OAuth 2.0 authentication will fail. No Drive access possible.',   fix:'Set GOOGLE_CLIENT_SECRET from Google Cloud Console → Credentials.' },
  { id:'dv-03', category:'Credentials', label:'GOOGLE_DRIVE_REFRESH_TOKEN set',     status:'fail',    detail:'Refresh token not configured. Required for persistent access.',  impact:'Drive sessions will expire and cannot be refreshed automatically.',fix:'Complete OAuth flow and store refresh token in GOOGLE_DRIVE_REFRESH_TOKEN.' },
...
export const DRIVE_GOVERNANCE_ISSUES: DriveGovernanceIssue[] = [
  ...
  { id:'dgi-04', severity:'High',     category:'Permission',      title:'Google Drive credentials not configured',         detail:'GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_DRIVE_REFRESH_TOKEN are all missing.',                 affectedObjects:['All Program Folders','All Drive Files'],                                                    resolution:'Configure Google Drive OAuth credentials in Replit environment secrets.', status:'Open' },
...
export const DRIVE_TEST_SUITES: DriveTestSuite[] = [
  {
    ...
    tests: [
      { id:'dt-01', name:'GOOGLE_CLIENT_ID present',       description:'Verify GOOGLE_CLIENT_ID is set.', status:'fail',    result:'Secret not found. Set GOOGLE_CLIENT_ID.' },
      { id:'dt-02', name:'GOOGLE_CLIENT_SECRET present',   description:'Verify GOOGLE_CLIENT_SECRET is set.', status:'fail', result:'Secret not found. Set GOOGLE_CLIENT_SECRET.' },
      { id:'dt-03', name:'Refresh token configured',       description:'Verify GOOGLE_DRIVE_REFRESH_TOKEN is set.', status:'fail', result:'Refresh token not found. Complete OAuth flow.' },
