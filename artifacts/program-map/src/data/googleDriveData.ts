...
export const DRIVE_VALIDATION_CHECKS: DriveValidationCheck[] = [
  ...
  { id:'dv-03', category:'Credentials', label:'GOOGLE_DRIVE_REFRESH_TOKEN set',     status:'pass',    detail:'GOOGLE_DRIVE_REFRESH_TOKEN obtained Jun 13 2026 via Google OAuth flow — stored in Replit Secrets. Replit Google Drive connector live.',        impact:'Drive API sessions can be established.',    fix:'' },
  { id:'dv-04', category:'OAuth',       label:'Drive API enabled in Google Cloud',  status:'warning', detail:'OAuth token obtained and Replit connector live. Drive API enablement not yet confirmed via direct REST call.',       impact:'Drive API calls may fail if API is not enabled in GCP.', fix:'Make a test Drive API call (list files in root) to confirm API is enabled and scopes are active.' },
  { id:'dv-05', category:'OAuth',       label:'OAuth consent screen configured',    status:'warning', detail:'OAuth flow completed and token obtained — consent screen was accepted. Internal app mode assumed.',         impact:'Needs review if expanding to external users.', fix:'Verify consent screen is configured for internal (Workspace) use.' },
  { id:'dv-06', category:'OAuth',       label:'Required scopes declared',           status:'warning', detail:'Token obtained but scopes (drive.readonly, drive.file, drive.metadata.readonly) not yet verified via live API call.', impact:'Missing scopes will cause permission errors on folder/file operations.', fix:'Wire first Drive folder read to confirm scope coverage — see Phase 2 Drive workspace panel.' },
  { id:'dv-07', category:'Permissions', label:'Read access to shared Drive',        status:'warning', detail:'Credentials configured. Shared Drive read access not yet tested via direct API call.',                impact:'Cannot confirm program folder reads until first Drive API call is made.',                   fix:'Wire /api/drive/files endpoint and test folder listing against /Transition Trails/Programs/.' },
  { id:'dv-08', category:'Permissions', label:'Write access for Penny sources',     status:'warning', detail:'Credentials configured. Write scope (drive.file) not yet verified — Phase 2 priority.',   impact:'Penny source material uploads blocked until write access confirmed.',          fix:'Test drive.file write scope when Penny source sync is wired in Phase 2.' },
  { id:'dv-09', category:'Folder Access', label:'Program root folder accessible',  status:'warning', detail:'Credentials live. Folder paths are defined in Trail OS but no live Drive API call has been made yet.',           impact:'Program Folder Registry cannot be confirmed from live Drive until first API call.',     fix:'Wire first program folder read — see /api/drive/files when ready.' },
  { id:'dv-10', category:'Folder Access', label:'Template folder accessible',       status:'warning', detail:'Credentials live. Template folder path defined; access not yet verified.',                           impact:'Template sync and Penny template sources unverified.',            fix:'Test /Transition Trails/Templates/ access when Drive API wired.' },
  { id:'dv-11', category:'File Access', label:'File listing available',             status:'warning', detail:'Credentials configured. File listing requires confirming folder access with a live Drive API call.',                       impact:'File & Asset Catalog cannot be confirmed until first Drive API call.',        fix:'Wire /api/drive/files folder listing in Phase 2.' },
  { id:'dv-12', category:'File Access', label:'File metadata readable',             status:'warning', detail:'Credentials configured. Metadata access (drive.metadata.readonly) requires a live API call to confirm.', impact:'Governance checks and staleness detection unconfirmed.', fix:'Verify metadata scope when Drive workspace panel is wired.' },
];
...
export const DRIVE_GOVERNANCE_ISSUES: DriveGovernanceIssue[] = [
  ...
  { id:'dgi-04', severity:'Medium',   category:'Permission',      title:'Google Drive refresh token — resolved',               detail:'GOOGLE_DRIVE_REFRESH_TOKEN obtained Jun 13 2026 via Google OAuth flow — stored in Replit Secrets. Replit Google Drive connector live. Next step: wire first Drive folder read to confirm scope coverage and folder access.',  affectedObjects:['All Program Folders'],                                                    resolution:'Resolved Jun 13 2026. Next step: wire /api/drive/files endpoint and confirm folder access in Phase 2 Drive workspace panel.', status:'Resolved' },
...
export const DRIVE_TEST_SUITES: DriveTestSuite[] = [
  {
    ...
    tests: [
      ...
      { id:'dt-03', name:'Refresh token configured',       description:'Verify GOOGLE_DRIVE_REFRESH_TOKEN is set.', status:'pass', result:'GOOGLE_DRIVE_REFRESH_TOKEN set Jun 13 2026. Replit Google Drive connector live. Token validity to be confirmed via first Drive API call.' },
      { id:'dt-04', name:'Drive API enabled',              description:'Verify Google Drive API is enabled in GCP.', status:'warning', result:'Token obtained — Drive API enablement not yet confirmed via direct REST call. Make a test list-files call to verify.' },
      { id:'dt-05', name:'OAuth consent screen',           description:'Verify OAuth consent screen is configured.', status:'warning', result:'OAuth flow completed successfully. Consent screen accepted — verify internal (Workspace) mode is set in GCP.' },
    ],
  },
  {
    ...
    tests: [
      { id:'dt-06', name:'Program root folder accessible', description:'Can list /Transition Trails/Programs/ contents.', status:'warning', result:'Credentials live. Folder access not yet tested — wire /api/drive/files to confirm.' },
      { id:'dt-07', name:'Foundations Trail folder',       description:'Can access /Programs/Foundations Trail/.', status:'warning', result:'Credentials live. FT folder path defined; access not yet confirmed via API.' },
      { id:'dt-08', name:'Guided Trail folder',            description:'Can access /Programs/Guided Trail/.', status:'warning', result:'Credentials live. GT folder path defined; access not yet confirmed via API.' },
      { id:'dt-09', name:'Templates folder',               description:'Can access /Transition Trails/Templates/.', status:'warning', result:'Credentials live. Templates path defined; access not yet confirmed via API.' },
      { id:'dt-10', name:'Penny Sources folder',           description:'Can read from /Transition Trails/Penny Sources/.', status:'warning', result:'Credentials live. Penny Sources path defined; access not yet confirmed via API.' },
    ],
  },
  {
    ...
    tests: [
      { id:'dt-11', name:'List files in program folder',    description:'Can enumerate files in a program folder.', status:'warning', result:'Credentials live. File listing requires confirming folder access — wire Drive API call to unblock.' },
      { id:'dt-12', name:'Read file metadata',              description:'Can read owner, modified date, permissions.', status:'warning', result:'Credentials live. Metadata access requires live Drive API call to confirm drive.metadata.readonly scope.' },
      { id:'dt-13', name:'Search by keyword',               description:'Can search Drive for files by keyword.', status:'warning', result:'Credentials live. Keyword search (Drive Files API) not yet tested — Phase 2 priority.' },
    ],
  },
  {
    ...
    tests: [
      { id:'dt-14', name:'Read permission — program files',  description:'Can read program curriculum documents.', status:'warning', result:'Credentials live. Read permissions require a live API call against program folders to confirm drive.readonly scope.' },
      { id:'dt-15', name:'Write permission — Penny sources', description:'Can write to designated Penny source folder.', status:'warning', result:'Credentials live. Write permissions (drive.file) not yet tested — Phase 2 Penny source sync priority.' },
      { id:'dt-16', name:'Metadata read permissions',        description:'drive.metadata.readonly scope active.', status:'warning', result:'Credentials live. Metadata scope not yet confirmed via live API call.' },
    ],
  },
  {
...
export const DRIVE_HEALTH_SCORES: DriveHealthScore[] = [
  { dimension:'credentials', label:'Credentials & OAuth', score:8, maxScore:10, status:'partial', note:'Refresh token live Jun 13 2026; Replit connector active. Scope coverage to confirm via first API call.', items:[{ label:'GOOGLE_CLIENT_ID', status:'pass', note:'Set and verified' },{ label:'GOOGLE_CLIENT_SECRET', status:'pass', note:'Set and verified' },{ label:'Refresh Token', status:'pass', note:'Obtained Jun 13 2026 · Replit connector live' },{ label:'Scope Verification', status:'partial', note:'Token live; drive.readonly / drive.file not yet confirmed via API call' }] },
  { dimension:'folder-access', label:'Folder Access', score:3, maxScore:10, status:'partial', note:'Credentials live; folder paths defined; access not yet confirmed via Drive API call', items:[{ label:'Foundations Trail Folder', status:'partial', note:'Path defined, not Drive-verified' },{ label:'Guided Trail Folder', status:'partial', note:'Path defined, not Drive-verified' },{ label:"Explorer's Trail Folder", status:'partial', note:'Path defined, not Drive-verified' },{ label:'Trail of Mastery Folder', status:'fail', note:'Folder not created in Drive' },{ label:'Digital Compass Folder', status:'fail', note:'Folder not created in Drive' }] },
  ...
  { dimension:'penny-sources', label:'Penny Source Readiness', score:5, maxScore:10, status:'partial', note:'Sources defined; credentials live — Drive file sync is Phase 2 priority', items:[{ label:'Resume Review Sources', status:'pass', note:'3 sources mapped' },{ label:'Weekly Reflection Sources', status:'pass', note:'2 sources mapped' },{ label:'Learner Coaching Sources', status:'partial', note:'3 sources mapped' },{ label:'Module Generation Sources', status:'partial', note:'Missing primary source' }] },
  { dimension:'permissions', label:'Permissions Readiness', score:3, maxScore:10, status:'partial', note:'Credentials live; read/write/metadata not yet confirmed via live Drive API call', items:[{ label:'Read Permissions', status:'partial', note:'Credentials live; drive.readonly not yet confirmed' },{ label:'Write Permissions', status:'partial', note:'drive.file scope not yet tested' },{ label:'Metadata Access', status:'partial', note:'drive.metadata.readonly not yet confirmed' }] },
];
