...
export const integrations: Integration[] = [
...

  {
    ...
    syncReadiness: mkChecks({
      'Source Exists':          { status: 'Pass',    notes: 'Salesforce REST API live via Replit Connector. pmdm__ProgramEngagement__c confirmed accessible — 36 engagement records returned including 2 Active learners (Marissa Pavlak, Michele Ward on Foundations Trail).' },
      ...
      'Object Mapping Done':    { status: 'Pass',    notes: 'pmdm__ managed-package objects confirmed accessible. Stage field is pmdm__Stage__c (not pmdm__ProgramStatus__c). Active learner directory now queries via pmdm__ProgramEngagement__c WHERE pmdm__Stage__c = Active. Learner__c is required on Penny_Interaction_Log__c.' },
      'Sync Direction Known':   { status: 'Pass',    notes: 'Bidirectional confirmed. Write scope: Penny_Interaction_Log__c only.' },
      'Error Handling Planned': { status: 'Partial', notes: 'Retry logic in place for Penny ask endpoint. No dead-letter queue designed yet.' },
    }),
    ...
    risks: ['Connected App callback URL needs registering before full user-facing OAuth flow works', 'Bidirectional sync scope risk: write permissions too broad without row-level security', 'pmdm__Stage__c picklist values must be kept in sync with Trail OS active-learner filter'],
    blockers: ['Connected App OAuth callback URL registration pending'],
    nextSteps: ['Register Connected App callback URL', 'Define row-level security for write scope', 'Expand learner directory to include Waitlisted stage when DM delivery is ready'],
    readinessScore: 72,
  },
