...
export const integrations: Integration[] = [
...

  {
    ...
    syncReadiness: mkChecks({
      'Source Exists':          { status: 'Pass',    notes: 'Salesforce REST API live via Replit Connector. 7/8 PMM objects accessible (pmdm__ namespace).' },
      'Owner Assigned':         { status: 'Pass',    notes: 'Salesforce Admin is the owner.' },
      'Permissions Known':      { status: 'Partial', notes: 'OAuth active via Replit Connector SDK. Connected App callback URL registration pending (task in progress).' },
      'Object Mapping Done':    { status: 'Partial', notes: 'Core objects mapped. Program_Engagement__c custom object not yet created in org — preflight returns 404.' },
    ...
    risks: ['Program_Engagement__c custom object not yet created — Resume Review capability blocked', 'Connected App callback URL needs registering before full OAuth flow works', 'Bidirectional sync scope risk: write permissions too broad without row-level security'],
    blockers: ['Program_Engagement__c custom object needs to be created in the Salesforce org', 'Connected App OAuth callback URL registration pending (task in progress)'],
    nextSteps: ['Create Program_Engagement__c custom object in Salesforce Setup', 'Register Connected App callback URL', 'Define row-level security for write scope'],
    readinessScore: 62,
  },
...

  {
    ...
    syncReadiness: mkChecks({
      ...
      'Permissions Known':      { status: 'Partial',     notes: 'OAuth active via Replit Connector SDK. Connected App callback URL registration pending.' },
    ...
    blockers: [],
    nextSteps: ['Register Connected App callback URL', 'Define Contact deduplication strategy', 'Confirm Case access scope with Operations Lead'],
    readinessScore: 75,
  },
...

  {
    ...
    syncReadiness: mkChecks({
      'Source Exists':          { status: 'Pass',        notes: 'Foundations Trail folder exists in shared Drive. General Drive OAuth active (GOOGLE_DRIVE_REFRESH_TOKEN). Penny Asset Library reads Shared Drive successfully.' },
      'Owner Assigned':         { status: 'Pass',        notes: 'Curriculum Lead.' },
      'Permissions Known':      { status: 'Partial',     notes: 'OAuth complete via /admin/integrations/google-auth. Shared Drive support (supportsAllDrives) confirmed. Foundations-specific folder not yet connected via Drive admin page.' },
      'Data Quality Verified':  { status: 'Not Started', notes: 'Folder structure not yet reviewed for Standards compliance.' },
      'Object Mapping Done':    { status: 'Partial',     notes: 'File → SourceDoc mapping designed but not tested for Foundations folder.' },
...

  {
    ...
    priority: 'P1',
    owner: 'Penny Product Lead',
    description: 'Gemini 2.5 Flash integration powering Penny\'s AI coaching responses. GEMINI_API_KEY active with billing confirmed (serviceTier: standard). POST /api/penny/ask endpoint live — Penny responds with Trail OS context injection, retry logic (3 attempts, 1s/2s backoff), and Penny_Interaction_Log__c write-back.',
    purpose: 'The AI engine that makes Penny intelligent. Gemini 2.5 Flash executes prompt templates with Trail OS context (learner, program, knowledge). POST /api/penny/ask is live and validated. Ask Penny panel returns real AI responses.',
    ...
    readinessScore: 78,
    pennyNote: 'Gemini 2.5 Flash is live — POST /api/penny/ask returns real AI responses with Trail OS context. Retry logic handles temporary overload. Ask Penny panel and Slack bot both use this endpoint.',
  },
