      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className={`text-[14px] font-bold  px-1.5 py-0.5 rounded border ${profile.categoryBg} ${profile.categoryColor}`}>
              {profile.category}
            </span>
            <span className="text-[14px] text-muted-foreground">{profile.objectTypeName}</span>
          </div>
          <h2 className="text-base font-bold text-foreground">{profile.name}</h2>
          <p className="text-[14px] text-muted-foreground leading-relaxed">{profile.description}</p>
        </div>
        ...
        <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${hCfg.bg}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[14px] text-muted-foreground truncate">{profile.health.summary.slice(0, 80)}…</p>
          </div>
        </div>
        ...
        {topInds.length > 0 && (
          <div className="space-y-1">
            <p className="text-[14px] font-bold  text-muted-foreground/60">Health Indicators</p>
            {topInds.map(ind => (
              <div key={ind.name} className="flex items-center gap-2">
                ...
                <span className="text-[14px] text-foreground flex-1">{ind.name}</span>
                <span className="text-[14px] font-medium text-foreground/80">{ind.value}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Ownership */}
        <div>
          <p className="text-[14px] font-bold  text-muted-foreground/60 mb-1">Owner</p>
          <p className="text-[14px] font-semibold text-[#2F6F7E]">{profile.ownership.primary}</p>
          <p className="text-[14px] text-muted-foreground">{profile.ownership.team}</p>
        </div>

        {/* Top relationships */}
        {topRels.length > 0 && (
          <div>
            <p className="text-[14px] font-bold  text-muted-foreground/60 mb-1">Relationships</p>
            <div className="space-y-1">
              {topRels.map((r, i) => (
                <div key={i} className="flex items-center gap-1.5">
                  <span className={`text-[14px] px-1 py-0.5 rounded border ${r.direction === 'upstream' ? 'bg-[#EDF5F8] border-[#7FAFC6] text-[#2F6F7E]' : 'bg-[#EDF5F8] border-[#7FAFC6] text-[#2F6F7E]'}`}>
                    {r.direction === 'upstream' ? '↑' : '↓'}
                  </span>
                  <span className="text-[14px] text-foreground truncate">{r.objectName}</span>
        ...
        <div className="space-y-1.5 pt-1 border-t border-border/40">
          <button
            ...
            className="flex items-center gap-1 text-[14px] font-semibold text-primary hover:underline w-full"
          >
            View Full Profile <ArrowRight className="w-3 h-3" />
          </button>
          <button
            onClick={() => setLocation(profile.workspaceLink)}
            className="flex items-center gap-1 text-[14px] text-muted-foreground hover:text-foreground w-full"
          >
      ...
      <div className="p-4 space-y-4">
        {uomType && (
          <div className="space-y-1.5">
            <span className="text-[14px] font-bold  text-muted-foreground/60">{uomType.category.replace('-layer', '').replace('-', ' ')} Layer</span>
            <h2 className="text-base font-bold text-foreground">{name}</h2>
            <p className="text-[14px] text-muted-foreground leading-relaxed">{uomType.purpose}</p>
          </div>
        )}
        {!uomType && (
          <div>
            <h2 className="text-base font-bold text-foreground">{name}</h2>
            {data?.description && <p className="text-[14px] text-muted-foreground">{data.description}</p>}
          </div>
        )}

        {data && Object.keys(data).filter(k => typeof data[k] === 'string' && k !== 'id').slice(0, 5).length > 0 && (
          <div className="space-y-1">
            <p className="text-[14px] font-bold  text-muted-foreground/60">Key Details</p>
            {Object.keys(data).filter(k => typeof data[k] === 'string' && k !== 'id').slice(0, 5).map(k => (
              <div key={k} className="flex items-baseline gap-1.5">
                <span className="text-[14px] text-muted-foreground/60 capitalize w-20 shrink-0">{k.replace(/([A-Z])/g,' $1').toLowerCase()}</span>
                <span className="text-[14px] text-foreground">{String(data[k]).slice(0, 60)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {uomType && (
          <>
            <div className="space-y-1">
              <p className="text-[14px] font-bold  text-muted-foreground/60">Owned By</p>
              {uomType.ownership.slice(0, 2).map(o => <p key={o} className="text-[14px] font-medium text-[#2F6F7E]">{o}</p>)}
            </div>
            <div className="space-y-1">
              <p className="text-[14px] font-bold  text-muted-foreground/60">Source of Truth</p>
              <p className="text-[14px] text-[#2F6B3F] font-medium">{uomType.sourceOfTruth}</p>
            </div>
