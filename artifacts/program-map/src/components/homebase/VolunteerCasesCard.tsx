 ...
 * VolunteerCasesCard
 *
 * Collapsible "Your Cases" card for the Volunteer Homebase.
 * Mirrors the pattern of CoachCasesCard but calls the volunteer cases endpoint.
 *
 * States:
 *   loading        — spinner
...
import type { VolunteerCasesState, VolunteerCase } from "@/hooks/useHomebaseVolunteer";
...
function CaseRow({ c }: { c: VolunteerCase }) {
  return (
    <div className="flex items-start gap-3 py-3 border-b border-border last:border-0">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[12px] text-muted-foreground mt-0.5">
          Updated {caseAge(c.LastModifiedDate)}
        </p>
      </div>
    </div>
  );
...
// ── VolunteerCasesCard (exported) ─────────────────────────────────────────────

interface VolunteerCasesCardProps {
  isLoading:  boolean;
  casesState: VolunteerCasesState | undefined;
  error:      Error | null;
}

export function VolunteerCasesCard({ isLoading, casesState, error }: VolunteerCasesCardProps) {
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0">
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-4">No open cases right now.</p>
          ) : (
            <div>{cases.map(c => <CaseRow key={c.Id} c={c} />)}</div>
          )}
        </div>
      )}
    </div>
