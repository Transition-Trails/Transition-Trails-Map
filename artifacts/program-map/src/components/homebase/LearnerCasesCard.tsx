 ...
 * LearnerCasesCard
 *
 * Collapsible "Your Cases" card for the Learner Homebase.
 *
 * States:
 *   loading      — spinner
 *   not-linked   — "Cases will appear here once your account setup is complete"
 *   all-closed   — compact green-tick row "All closed — N delivered"
...
import type { CasesState, LearnerCase } from "@/hooks/useHomebaseLearner";
...
function CaseRow({ c }: { c: LearnerCase }) {
  return (
...
// ── LearnerCasesCard (exported) ────────────────────────────────────────────────

interface LearnerCasesCardProps {
  isLoading:  boolean;
  casesState: CasesState | undefined;
  error:      Error | null;
}

export function LearnerCasesCard({ isLoading, casesState, error }: LearnerCasesCardProps) {
  const [expanded, setExpanded] = useState(true);

  const linked       = casesState?.linked;          // boolean | null | undefined
  const sfUnavailable = casesState?.sfUnavailable;
  const cases        = casesState?.cases ?? [];
  const totalOpen    = casesState?.totalOpen ?? 0;
  // All-closed: only when we know the account is linked AND cases came back empty
  const allClosed    = linked === true && cases.length === 0 && !isLoading && !error;

  // Compact "all closed" state
  if (allClosed) {
    return (
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0">
          {isLoading ? (
            ...
            // SF credentials absent or temporarily unreachable — distinct from unlinked account
            ...
            // SF is up but this email has no Contact record yet
            <p className="text-sm text-muted-foreground py-4 leading-relaxed">
