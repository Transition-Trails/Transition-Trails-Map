  ...
  if (saved) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 bg-background">
        ...
        <p className="text-[14px] text-muted-foreground text-center max-w-xs leading-relaxed">
          Your {objectType} was added in prototype mode. Data resets on page refresh until connected to a live backend.
        </p>
        <span className="text-[14px] font-bold  text-[#CC8400] border border-[#FFD08A] bg-[#FFF3E0] rounded-full px-2.5 py-1 mt-1">
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-background">
      ...
      <div className="px-5 pt-4 pb-3 border-b border-border flex-shrink-0">
        <div className="flex items-start gap-3 mb-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            {subtitle && (
              <p className="text-[14px] text-muted-foreground mt-0.5 leading-snug">{subtitle}</p>
            )}
          </div>
          <div className="flex flex-col items-end gap-1 shrink-0">
            ...
            <span className="text-[14px] text-muted-foreground/50">Prototype only</span>
          </div>
        </div>
      </div>
      ...
      <ScrollArea className="flex-1">
        <div className="p-5 space-y-5 max-w-2xl">
            ...
            <div key={field.id} className="space-y-1">
              ...
              {field.type === 'text' && (
                <Input
                  ...
                  className="h-8 text-[14px] bg-white"
                />
              )}
              ...
              {field.type === 'textarea' && (
                <textarea
                  ...
                  placeholder={field.placeholder ?? `Enter ${field.label.toLowerCase()}…`}
                  rows={field.rows ?? 4}
                  className="w-full rounded-md border border-input bg-white px-3 py-2 text-[14px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring resize-y min-h-[80px]"
                />
              )}
              ...
              {field.type === 'select' && field.options && (
                <select
                  value={values[field.id] ?? ''}
                  onChange={e => set(field.id, e.target.value)}
                  className="w-full h-8 rounded-md border border-input bg-white px-2.5 text-[14px] text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                >
          ...
          <div className="rounded-lg border border-border bg-muted/30 px-3 py-2.5">
            <p className="text-[14px] font-bold text-muted-foreground/60  mb-0.5">Auto-assigned on creation</p>
            <div className="flex gap-3 text-[14px] text-foreground">
      ...
      <div className="px-5 py-3 border-t border-border bg-background flex-shrink-0 flex items-center gap-2">
        <button
          onClick={onClose}
          className="px-3 py-1.5 text-[14px] font-semibold text-muted-foreground border border-border rounded-full hover:bg-muted/40 transition-colors"
        >
          Cancel
        </button>
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 text-[14px] font-semibold border border-border rounded-full hover:bg-muted/40 transition-colors"
          >
            Save as Draft
          </button>
          <button
            onClick={handleSaveAndView}
            className="px-4 py-1.5 text-[14px] font-bold bg-foreground text-background rounded-full hover:opacity-90 transition-opacity"
          >
            Save &amp; View
          </button>
        </div>
