    ...
    <PanelCard>
      ...
      {isLoading && (
        <div className="flex items-center gap-2 py-2">
          ...
          <span className="text-[14px] text-muted-foreground">Loading from Salesforce…</span>
        </div>
      )}
      {isError && (
        <p className="text-[14px] text-muted-foreground/60 italic">Could not load programs — SF connection unavailable.</p>
      )}
      {data && (
        <>
          ...
          <div className="space-y-1.5">
            {data.programs.slice(0, 6).map(p => (
              <div key={p.Id} className="flex items-center gap-2.5 py-1.5 border-b border-border/40 last:border-0">
                ...
                <span className="text-[14px] text-foreground flex-1 truncate">{p.Name}</span>
                {p.pmdm__Status__c && (
                  <span className="text-[14px] text-muted-foreground shrink-0">{p.pmdm__Status__c}</span>
                )}
              </div>
            ))}
            {data.programs.length > 6 && (
              <p className="text-[14px] text-muted-foreground/60 pt-0.5">
  ...
  return (
    <PanelCard>
      ...
      {isLoading && (
        <div className="flex items-center gap-2 py-2">
          <Loader2 className="w-3.5 h-3.5 animate-spin text-muted-foreground" />
          <span className="text-[14px] text-muted-foreground">Loading weekly reports…</span>
        </div>
      )}
      {!isLoading && reports.length === 0 && (
        <p className="text-[14px] text-muted-foreground/60 italic">No weekly intelligence reports yet. Reports are generated by {TERMS.aiAssistant} after each program week.</p>
      )}
      {reports.length > 0 && (
          ...
          {reports.map((r, i) => {
            ...
            return (
              <div key={r.id ?? i} className="rounded-md border border-border/60 bg-muted/10 px-3 py-2.5">
                ...
                {r.topThemes && (
                  <div className="mb-1">
                    <p className="text-[14px] font-medium text-muted-foreground  mb-0.5">Top Themes</p>
                    <p className="text-[14px] text-foreground/80 leading-snug line-clamp-2">{r.topThemes}</p>
                  </div>
                )}
                {r.supportFlags && (
                  <div>
                    <p className="text-[14px] font-medium text-[#CC8400]  mb-0.5">Support Flags</p>
                    <p className="text-[14px] text-foreground/80 leading-snug line-clamp-2">{r.supportFlags}</p>
                  </div>
                )}
              </div>
            );
...
function CasesPanel() {
  ...
  return (
    <PanelCard>
      ...
      {isError && (
        <p className="text-[14px] text-muted-foreground/60 italic">Could not load cases — SF connection unavailable.</p>
      )}
      {!isLoading && !isError && (
        <>
          <p className="text-[14px] text-muted-foreground">
            ...
            <div className="space-y-1.5">
              {openCases.map(c => (
                <div key={c.Id} className="flex items-center gap-2.5 py-1.5 border-b border-border/40 last:border-0">
                  ...
                  <span className="text-[14px] text-foreground flex-1 truncate">{c.Subject ?? `Case ${c.CaseNumber}`}</span>
                  {c.Status && (
                    <span className="text-[14px] text-muted-foreground shrink-0">{c.Status}</span>
                  )}
                </div>
              ))}
            </div>
          ...
          <button
            ...
            className="flex items-center gap-1 text-[14px] font-medium text-primary hover:underline mt-1"
          >
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-5 space-y-5 max-w-3xl">
        ...
        <div>
          <p className="text-[14px] font-bold  text-muted-foreground/50 mb-2 mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {FUTURE_SECTIONS.map(s => (
              <div key={s.title} className="rounded-lg border border-border bg-background p-3">
                <p className="text-[14px] font-semibold text-foreground mb-0.5">{s.title}</p>
