...
import { useKnowledgeArticles, type KnowledgeArticle, type ArticleFormData } from '@/hooks/useKnowledgeArticles';
...
function EditForm({ article, onSave, onSubmit, onDelete, saving }: EditFormProps) {
  ...
  async function handleSave() {
    await onSave({ title, summary, body, category, urlName: slugify(title) });
    setDirty(false);
  }

  const isNew    = !article;
  const canSubmit = !isNew && article.status === 'draft' && title.trim().length > 0;
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-5 py-3 border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : isNew ? 'Create Draft' : 'Save'}
          </button>
          {canSubmit && (
            <button
              onClick={onSubmit}
              disabled={saving || dirty}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-amber-300 bg-amber-50 text-amber-700 text-[12px] font-medium hover:bg-amber-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
        ...
        <div className="p-5 space-y-4 max-w-2xl">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={18}
              className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none font-mono"
            />
            <p className="text-[11px] text-muted-foreground/70">HTML is supported (headings, lists, bold, links).</p>
          </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1">
        <div className="p-5 space-y-5 max-w-2xl">
          ...
          {article.body ? (
            <div
              className="prose prose-sm max-w-none text-foreground rounded-lg border border-border bg-background p-4 text-[14px] leading-relaxed"
      ...
      <ScrollArea className="flex-1">
        <div className="p-5 space-y-4 max-w-2xl">
          ...
          <div className="rounded-lg border border-border bg-background p-4 space-y-2">
            ...
            <div className="pt-1 flex items-center gap-3 text-[12px] text-muted-foreground/70">
              <span>Approved {fmt(article.reviewedAt)}</span>
              {article.reviewedBy && <span>by {article.reviewedBy}</span>}
            </div>
          </div>
            ...
            <div className="space-y-3">
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 space-y-1.5">
                ...
                <p className="text-[13px] text-emerald-700">
                  ...
                  SF status: <strong>{article.sfPublishStatus ?? 'Draft'}</strong>.
                </p>
              </div>
              {article.sfArticleId && (
                <div className="rounded-lg border border-border bg-background p-3 space-y-1">
                  <p className="text-[11px] font-semibold text-muted-foreground tracking-wide uppercase">Salesforce IDs</p>
                  <p className="text-[12px] text-foreground font-mono">Article: {article.sfArticleId}</p>
                  {article.sfVersionId && (
                    <p className="text-[12px] text-foreground font-mono">Version: {article.sfVersionId}</p>
                  )}
                </div>
              )}
              <p className="text-[12px] text-muted-foreground/70 flex items-center gap-1">
                <ExternalLink className="w-3 h-3" />
                View and manage this article in Salesforce Knowledge.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-2">
                ...
                <ul className="space-y-1.5">
                  {[
                    'A new Knowledge Article record is created in Salesforce.',
                    'The article title, summary, category, and body are sent to Salesforce.',
                    'Salesforce will create the article as a Draft. If your org allows it, it will be published immediately.',
                    'The Salesforce article ID is saved here so you can track it.',
                  ].map((s, i) => (
...
export default function ArticleStudio() {
  ...
  async function wrap(fn: () => Promise<unknown>) {
    ...
    try {
      ...
      showToast(e instanceof Error ? e.message : 'An error occurred', false);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleRequestChanges(note: string) {
    if (!selectedId) return;
    await wrap(async () => {
      await requestChanges(selectedId, note);
      showToast('Changes requested — article returned to author');
    });
  }
  ...
  async function handlePublish() {
    ...
    await wrap(async () => {
      ...
      showToast('Article published to Salesforce');
    });
  }
  ...
  // ── determine which right-panel view to show ──
  const showEdit    = isNew || (selected && (selected.status === 'draft'));
  const showReview  = selected && selected.status === 'pending-review';
  const showPublish = selected && (selected.status === 'approved' || selected.status === 'published');
  ...
  return (
    <div className="flex h-full overflow-hidden relative">
      ...
      <div className="w-64 shrink-0 border-r border-border flex flex-col bg-background">
        ...
        {/* Status filter tabs */}
        <div className="flex gap-0.5 px-2 pt-2 pb-1 flex-wrap">
          {FILTER_TABS.map(tab => {
