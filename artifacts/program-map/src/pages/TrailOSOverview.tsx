    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 flex items-start justify-between px-5 pt-3 pb-2.5 border-b bg-card">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50 mb-0.5">Platform Overview</p>
          <h1 className="text-[15px] font-semibold text-foreground leading-snug">Trail OS Overview</h1>
          <p className="text-[12px] text-muted-foreground mt-0.5 max-w-lg leading-relaxed">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-4 mt-0.5">
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold text-muted-foreground bg-white border border-border/70 hover:text-foreground hover:border-border transition-colors"
          >
        ...
        {TABS.map(tab => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium rounded-t transition-colors border-b-2 -mb-px ${
                active
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 max-w-5xl space-y-5">
          ...
          {activeTab === 'capabilities' && (
            <>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50 mb-2.5">What Trail OS Does</p>
                <div className="grid grid-cols-2 gap-2.5">
                  {PLATFORM_AREAS.map(area => (
                    <div key={area.id} className={`p-3 rounded-lg border ${area.ring} ${area.bg}`}>
                      <p className={`text-[11px] font-bold uppercase tracking-wide mb-1 ${area.tag}`}>{area.label}</p>
                      <p className="text-[12px] text-muted-foreground leading-snug">{area.description}</p>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {trailOsCapabilities.map(cap => (
                    ...
                    >
                      <div className="flex items-start gap-2 mb-1">
                        ...
                        <p className="text-[12px] font-semibold text-foreground leading-snug">{cap.name}</p>
                      </div>
                      <p className="text-[11px] text-muted-foreground leading-snug line-clamp-2 pl-5">{cap.description}</p>
                      {cap.programs.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1.5 pl-5">
                          ...
                          {cap.programs.length > 2 && (
                            <Badge variant="secondary" className="text-[9px] px-1.5 py-0">+{cap.programs.length - 2}</Badge>
                          )}
                        </div>
                      )}
                      <div className="flex items-center gap-1 mt-1.5 pl-5 text-[10px] text-muted-foreground/50 group-hover:text-primary transition-colors">
          ...
          {activeTab === 'delivery' && (
            <>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50 mb-0.5">Learner Journey</p>
                <p className="text-[12px] text-muted-foreground">End-to-end delivery loop from intake to alumni engagement.</p>
              </div>
                  ...
                  {DELIVERY_STEPS.map(({ step }, i) => (
                    <div key={step} className="flex items-center">
                      <div className="flex flex-col items-center text-center w-24">
                        ...
                        <span className="text-[11px] font-medium text-foreground leading-tight">{step}</span>
                      </div>
                ...
                {DELIVERY_STEPS.map(({ step, detail }, i) => (
                  <div key={step} className="p-3 rounded-lg border bg-white/80 border-border/60">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="text-[10px] font-bold bg-primary/10 text-primary w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0">
                        {i + 1}
                      </span>
                      <p className="text-[12px] font-semibold text-foreground">{step}</p>
                    </div>
                    <p className="text-[11px] text-muted-foreground leading-snug pl-7">{detail}</p>
                  </div>
                ))}
              </div>
            </>
          ...
          {activeTab === 'analytics' && (
            <>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50 mb-0.5">Platform Metrics</p>
                <p className="text-[12px] text-muted-foreground">Illustrative planning data — not live. Live metrics will connect in Phase 2.</p>
              </div>
