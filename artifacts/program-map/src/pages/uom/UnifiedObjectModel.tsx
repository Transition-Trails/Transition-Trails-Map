...
function CatBadge({ category }: { category: ObjectCategory }) {
  ...
  if (!cat) return null;
  return (
    <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[14px] font-bold  border ${cat.bg} ${cat.color}`}>
      {cat.label}
    </span>
  );
}
    ...
    <ScrollArea className="h-full">
      <div className="p-6 space-y-6">
        ...
        <div className="rounded-lg border border-[#FFD08A] bg-[#FFF3E0] px-4 py-3">
          <p className="text-[14px] font-bold text-[#CC8400]  mb-1">Phase 1 Architecture Capability</p>
        ...
        <div>
          <p className="text-[14px] font-bold text-foreground  mb-3">Object Layers ({OBJECT_TYPES.length} Core Types)</p>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {OBJECTS_BY_CATEGORY.map(cat => (
              <div key={cat.id} className={`rounded-lg border p-3 ${cat.bg}`}>
                <p className={`text-[14px] font-bold  mb-1.5 ${cat.color}`}>{cat.label}</p>
                <div className="flex flex-wrap gap-1">
                  {cat.objects.map(o => (
                    <span key={o.id} className="text-[14px] text-foreground bg-white/70 rounded px-1.5 py-0.5 border border-white/80">
                      {o.name}
                    </span>
                  ))}
                </div>
        ...
        <div>
          <p className="text-[14px] font-bold text-foreground  mb-3">Common Object Profile Structure</p>
            ...
            <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
              {['Overview', 'Relationships', 'Ownership', 'Health', 'Standards', 'Related Programs',
                ...
                <div key={tab} className="flex items-center gap-1.5 px-2 py-1.5 rounded bg-muted/30 border border-border/60">
                  ...
                  <span className="text-[14px] text-foreground">{tab}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          <p className="text-[14px] font-bold text-foreground  mb-3">Relationship Principles</p>
            ...
            {[
              ...
              <div key={title} className="flex items-start gap-2.5 px-3 py-2 rounded-lg bg-muted/20 border border-border/60">
                ...
                <div>
                  ...
                  <span className="text-[14px] text-muted-foreground ml-1">{desc}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {OBJECTS_BY_CATEGORY.map(cat => (
          <div key={cat.id}>
            <div className="flex items-center gap-2 mb-2">
              <span className={`text-[14px] font-bold  ${cat.color}`}>{cat.label}</span>
              <span className="text-[14px] text-muted-foreground/60">{cat.objects.length} types</span>
            </div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">
              {cat.objects.map(obj => (
                ...
                >
                  <p className="text-[14px] font-bold text-foreground mb-0.5">{obj.name}</p>
                  <p className="text-[14px] text-muted-foreground leading-relaxed line-clamp-2 mb-2">{obj.purpose}</p>
                  <div className="flex flex-wrap gap-1 mt-auto">
                    <span className="text-[14px] bg-white/70 border border-white/80 rounded px-1 py-0.5 text-muted-foreground truncate max-w-[120px]">
                      SoT: {obj.sourceOfTruth.split(' ')[0]}
                    </span>
                    <span className="text-[14px] bg-white/70 border border-white/80 rounded px-1 py-0.5 text-muted-foreground">
...
function RelationshipMatrix() {
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-4">
        <div className="mb-3">
          <p className="text-[14px] font-bold text-foreground mb-1">Object-to-Object Relationship Matrix</p>
          <p className="text-[14px] text-muted-foreground">Rows = source object. Columns = target object. Cells show relationship type from source to target.</p>
        </div>

        <div className="flex flex-wrap gap-1.5 mb-4">
          {Object.entries(REL_ABBREV).map(([type, abbrev]) => (
            <div key={type} className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[14px] font-bold ${REL_CELL_COLOR[type]}`}>
              <span>{abbrev}</span>
              ...
              <tr>
                ...
                {OBJECT_TYPES.map(col => (
                  <th key={col.id} className="border border-border/40 bg-muted/20 px-0.5 py-1 min-w-[30px]">
                    <div className="writing-mode-vertical text-[14px] font-bold text-muted-foreground text-center" title={col.name}>
                      {SHORT_NAMES[col.id]}
                    </div>
                  </th>
                ))}
            ...
            <tbody>
              {OBJECT_TYPES.map(row => (
                <tr key={row.id} className="hover:bg-muted/10">
                  <td className="border border-border/40 bg-muted/20 px-1.5 py-0.5 text-[14px] font-bold text-muted-foreground whitespace-nowrap">
                    {SHORT_NAMES[row.id]}
                  </td>
                  {OBJECT_TYPES.map(col => {
                    ...
                    return (
                      <td key={col.id} className="border border-border/30 text-center p-0.5">
                        {relType ? (
                          <span className={`inline-flex items-center justify-center w-full h-5 rounded text-[14px] font-bold ${REL_CELL_COLOR[relType] ?? 'bg-muted'}`}>
                            {REL_ABBREV[relType] ?? '?'}
                          </span>
                        ) : (
    ...
    <ScrollArea className="h-full">
      <div className="p-4 space-y-4">
        <div className="rounded-lg border border-border bg-white">
          <div className="grid grid-cols-[1fr_200px_200px] text-[14px] font-bold text-muted-foreground  px-4 py-2 border-b border-border bg-muted/20">
            <div>Object Type</div>
            ...
            return (
              <div key={obj.id} className={`grid grid-cols-[1fr_200px_200px] px-4 py-2.5 ${i < OBJECT_TYPES.length - 1 ? 'border-b border-border/40' : ''} hover:bg-muted/10`}>
                <div className="flex items-center gap-2">
                  <span className="text-[14px] font-semibold text-foreground">{obj.name}</span>
                  {cat && <span className={`text-[14px] font-bold ${cat.color}`}>{cat.label.replace(' Layer', '')}</span>}
                </div>
                <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {obj.systems.slice(0, 3).map(s => (
                    <span key={s} className="text-[14px] bg-muted/30 border border-border/60 rounded px-1 py-0.5 text-muted-foreground">{s}</span>
                  ))}
                  {obj.systems.length > 3 && (
                    <span className="text-[14px] text-muted-foreground/60">+{obj.systems.length - 3}</span>
                  )}
                </div>
              </div>
            );
        ...
        <div>
          <p className="text-[14px] font-bold text-foreground  mb-2">By Authoritative System</p>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {systemGroups.map(g => (
              <div key={g.sys} className="rounded-lg border border-border bg-white p-3">
                <p className="text-[14px] font-bold text-[#2F6B3F] mb-1.5">{g.sys}</p>
                <div className="space-y-0.5">
                  {g.objects.map(o => (
                    <p key={o.id} className="text-[14px] text-foreground">{o.name}</p>
                  ))}
                </div>
              </div>
            ))}
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-4 space-y-3">
        <div className="rounded-lg border border-border bg-white">
          <div className="grid grid-cols-[1fr_160px_180px_160px] text-[14px] font-bold text-muted-foreground  px-4 py-2 border-b border-border bg-muted/20">
            <div>Object Type</div>
          ...
          {OBJECT_TYPES.map((obj, i) => (
            <div key={obj.id} className={`grid grid-cols-[1fr_160px_180px_160px] px-4 py-2.5 ${i < OBJECT_TYPES.length - 1 ? 'border-b border-border/40' : ''} hover:bg-muted/10`}>
              ...
              <div>
                <span className="text-[14px] text-foreground">{obj.ownership[0]}</span>
                {obj.ownership.length > 1 && (
                  <span className="text-[14px] text-muted-foreground"> + {obj.ownership.length - 1}</span>
                )}
              </div>
              <div>
                <span className="text-[14px] text-muted-foreground">{obj.reviewCycle}</span>
              </div>
              <div className="flex flex-wrap gap-1">
                {obj.standards.length > 0
                  ? obj.standards.map(s => (
                    <span key={s} className="text-[14px] bg-[#EDF5F8] border border-[#7FAFC6] text-[#2F6F7E] rounded px-1 py-0.5">{s}</span>
                  ))
        ...
        <div className="rounded-lg border border-border bg-white p-4">
          <p className="text-[14px] font-bold text-foreground  mb-2">Governance Principles</p>
      ...
      <div className="w-44 flex-shrink-0 border-r border-border overflow-y-auto py-2 px-1.5 bg-muted/10">
        {OBJECTS_BY_CATEGORY.map(cat => (
          <div key={cat.id} className="mb-2">
            <p className={`text-[14px] font-bold  px-2 py-1 ${cat.color}`}>{cat.label.replace(' Layer', '')}</p>
            {cat.objects.map(o => (
              <button
                key={o.id}
                onClick={() => { setSelectedId(o.id); setActiveTab('overview'); }}
                className={`w-full text-left px-2 py-1.5 rounded-md text-[14px] transition-colors ${
                  selectedId === o.id
            ...
            <div className="flex items-start justify-between mb-1">
              <div>
                <div className="flex items-center gap-2 mb-0.5">
                  <CatBadge category={obj.category} />
                  <span className="text-[14px] text-muted-foreground">Source of Truth: <strong>{obj.sourceOfTruth}</strong></span>
                </div>
                <h2 className="text-lg font-bold text-foreground">{obj.name}</h2>
                <p className="text-[14px] text-muted-foreground mt-0.5 leading-relaxed max-w-xl">{obj.purpose}</p>
              </div>
              <button
                onClick={() => setLocation(obj.workspaceLink)}
                className="flex items-center gap-1 text-[14px] text-primary hover:underline font-medium ml-4 flex-shrink-0 mt-1"
              >
            ...
            <div className="flex items-center gap-1 mt-2 mb-3">
              {obj.lifecycle.map((stage, i) => (
                <div key={stage} className="flex items-center gap-0.5">
                  <span className={`text-[14px] font-medium px-2 py-0.5 rounded-full border ${
            ...
            <div className="flex gap-0 border-b-0">
              {explorerTabs.map(tab => (
                <button
                  ...
                  className={`px-3 py-1.5 text-[14px] font-medium border-b-2 transition-colors ${
            ...
            <div className="p-5">
              {activeTab === 'overview' && (
                <div className="space-y-4">
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-2">Ownership</p>
                    <div className="flex flex-wrap gap-1.5">
                      {obj.ownership.map(o => (
                        <span key={o} className="text-[14px] px-2 py-1 rounded-md bg-[#EDF5F8] border border-[#7FAFC6] text-[#2F6F7E] font-medium">{o}</span>
                      ))}
                    </div>
                  </div>
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-2">Review Cycle</p>
                    <p className="text-[14px] text-foreground">{obj.reviewCycle}</p>
                  </div>
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-2">Connected Systems</p>
                    <div className="flex flex-wrap gap-1.5">
                      {obj.systems.map(s => <SoTBadge key={s} sys={s} />)}
                    </div>
                  </div>
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-2">Profile Tabs (Universal Object Profile)</p>
                    <div className="flex flex-wrap gap-1">
                      {obj.profileTabs.map(t => (
                        <span key={t} className="text-[14px] px-1.5 py-0.5 rounded bg-muted/40 border border-border/60 text-foreground/70">{t}</span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              {activeTab === 'relationships' && (
                <div className="space-y-2">
                  <p className="text-[14px] font-bold text-muted-foreground  mb-3">{obj.relationships.length} Direct Relationships</p>
                  {obj.relationships.map(rel => {
                    ...
                    return (
                      <div key={`${rel.targetId}`} className="flex items-start gap-3 px-3 py-2.5 rounded-lg border border-border bg-white hover:bg-muted/10">
                        <div className={`text-[14px] font-bold px-1.5 py-0.5 rounded border shrink-0 mt-0.5 ${REL_CELL_COLOR[rel.type] ?? 'bg-muted'}`}>
                          {relCfg?.label ?? rel.type}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[14px] font-semibold text-foreground">{rel.targetName}</p>
                          <p className="text-[14px] text-muted-foreground">{rel.description}</p>
                        </div>
                        {target && (
                          <button
                            onClick={() => setSelectedId(rel.targetId)}
                            className="text-[14px] text-primary hover:underline font-medium shrink-0 flex items-center gap-0.5 mt-0.5"
                          >
              ...
              {activeTab === 'ownership' && (
                <div className="space-y-4">
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-2">Owner Roles</p>
                    {obj.ownership.map((owner, i) => (
                      <div key={owner} className="flex items-center gap-3 px-3 py-2 rounded-lg border border-[#7FAFC6] bg-[#EDF5F8] mb-1.5">
                        <div className="w-5 h-5 rounded-full bg-[#2F6F7E] flex items-center justify-center text-[14px] font-bold text-white shrink-0">{i + 1}</div>
                        <div>
                          <p className="text-[14px] font-semibold text-[#2F6F7E]">{owner}</p>
                          <p className="text-[14px] text-[#2F6F7E]">{i === 0 ? 'Primary owner — final accountability' : 'Secondary owner — operational responsibility'}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-1.5">Review Cadence</p>
              ...
              {activeTab === 'health' && (
                <div className="space-y-2">
                  <p className="text-[14px] font-bold text-muted-foreground  mb-3">{obj.health.length} Health Indicators</p>
                  {obj.health.map(h => (
                    <div key={h.name} className="flex items-start gap-3 px-3 py-2.5 rounded-lg border border-border bg-white">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span className="text-[14px] bg-[#E6F0EA] border border-[#9FC3AE] text-[#2F6B3F] rounded px-1 py-0.5">{h.source}</span>
                        </div>
                        <p className="text-[14px] text-muted-foreground">{h.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {activeTab === 'standards' && (
                <div className="space-y-3">
                  <p className="text-[14px] font-bold text-muted-foreground  mb-2">Governing Standards</p>
                  {obj.standards.length > 0 ? (
                    obj.standards.map(s => (
                      <div key={s} className="px-3 py-2.5 rounded-lg border border-[#7FAFC6] bg-[#EDF5F8]">
                        <p className="text-[14px] font-semibold text-[#2F6F7E]">{s}</p>
                        <p className="text-[14px] text-[#2F6F7E] mt-0.5">View full standard in Standards Studio → Program & Curriculum workspace</p>
                      </div>
                    ))
                  ) : (
                    <div className="px-3 py-3 rounded-lg border border-border bg-muted/20 text-center">
                      <p className="text-[14px] text-muted-foreground">No governing standards defined for this object type.</p>
                      <p className="text-[14px] text-muted-foreground/60 mt-0.5">This object type follows general organizational guidelines.</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {activeTab === 'systems' && (
                <div className="space-y-3">
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-1.5">Source of Truth</p>
                    <div className="px-3 py-2.5 rounded-lg border border-[#9FC3AE] bg-[#E6F0EA]">
                      <p className="text-[14px] font-bold text-[#245531]">{obj.sourceOfTruth}</p>
                      <p className="text-[14px] text-[#2F6B3F] mt-0.5">The authoritative system of record for this object type.</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-[14px] font-bold text-muted-foreground  mb-1.5">Connected Systems</p>
                    <div className="space-y-1.5">
                      {obj.systems.map(s => (
    ...
    <ScrollArea className="h-full">
      <div className="p-4 space-y-4">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-sm text-muted-foreground whitespace-nowrap">{filtered.length} of {OBJECT_TYPES.length}</span>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {catConfig && (
                      <span className="text-[14px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{catConfig.label}</span>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground truncate mt-0.5">{obj.purpose}</p>
                </div>
