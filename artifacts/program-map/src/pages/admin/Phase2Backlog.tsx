...
const CARDS: BacklogCard[] = [
  ...
  },
  {
    ...
    phase: 'Phase 2',
    owner: 'TBD',
    summary: 'Google Sign-In via Clerk v6 is live — branded /sign-in page, Google OAuth, and role-gated routes. Remaining work: wire Google Admin Directory API so group membership (trailosadmin, trailospennyadmin, trailosusers) auto-assigns tiers on sign-in.',
    ...
    dependencies: [
      ...
      '⏳ GOOGLE_ADMIN_CREDENTIALS secret — service account with Google Admin Directory API scope (groups.readonly)',
      '⏳ Super Admin UI for configuring group → tier mapping without a code deploy',
    ],
    roleImpact: [
      { role: 'Everyday User',    impact: 'Google SSO already live — role auto-assignment from group membership activates when Directory API token is configured' },
      { role: 'Penny Power User', impact: 'trailospennyadmin group membership will grant Power tier capabilities on next sign-in' },
      { role: 'Admin',            impact: 'trailosadmin group membership will grant Admin tier on next sign-in; can view tier assignments in People & Access' },
      { role: 'Super Admin',      impact: 'Configures GOOGLE_ADMIN_CREDENTIALS and group → tier mapping table in Admin Settings' },
    ],
    acceptanceCriteria: [
      ...
      '⏳ Role mapping table configurable by Super Admin without a code deploy',
