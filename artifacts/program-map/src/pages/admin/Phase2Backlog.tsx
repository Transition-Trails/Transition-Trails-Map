...
const CARDS: BacklogCard[] = [
  ...
  },
  {
    ...
    phase: 'Phase 2',
    owner: 'Tech Lead',
    summary: 'Fully shipped. Org-level rules in Collaboration Overview (/collaboration). Personal control center at /collaboration/my-signals — source toggles (Penny/Slack/Gmail/Calendar/Salesforce) with urgency (High/Medium/Low) and delivery (Real-time/Digest) per source, expandable signal lists, personal watch rule builder (add/pause/delete), required-signal locks, and in-memory save. DB persistence deferred to a future phase.',
    tags: ['Trail Signals', 'Personalization', 'UX'],
    icon: Rss,
    hue: 'amber',
    dependencies: [
      '✓ Trail Signals source registry — Slack, Gmail, Calendar channel rules defined in Collaboration Overview',
      '✓ Signal destinations configured — Penny, Trail Signals, Notifications, Templates routing live',
      'User profile persistence layer (database-backed, not in-memory) — Phase 2',
      'Notification delivery infrastructure (email or push) — Phase 2',
      'Personal watch rule engine with conditional logic — Phase 2',
    ],
    roleImpact: [
      ...
      { role: 'Admin',            impact: '✓ Org-level signal rules now configurable in Collaboration Overview; user-level view is Phase 2' },
      { role: 'Super Admin',      impact: 'Can define which signal sources are available per role tier' },
    ],
    acceptanceCriteria: [
      '✓ Org-level channel signal rules visible and structured (Slack, Gmail, Calendar, Drive) — Collaboration Overview',
      '✓ Signal destinations defined — Penny, Trail Signals, Notifications, Templates routing configured',
      '✓ User can add, pause, and remove signal sources from personal feed — /collaboration/my-signals',
      '✓ Urgency levels (High / Medium / Low) configurable per signal type',
      '✓ Digest (daily summary) vs real-time toggle per signal source',
      '✓ Required signals (set by Admin/Penny) locked — cannot be toggled by user',
      '✓ Watch rules with conditions (object type, field, condition, threshold, action) — add/pause/delete',
      'Control Center preferences persist across sessions — deferred (needs DB, future phase)',
    ],
  },
  {
    id: 'p2-gmail-panel',
    ...
    phase: 'Phase 2',
    owner: 'Tech Lead',
    summary: 'GmailCenter live at /collaboration/gmail — real inbox (15 threads), thread read, Penny-drafted replies via Gemini, and real send via POST /api/gmail/send. GOOGLE_GMAIL_REFRESH_TOKEN active; gmail.readonly + gmail.send scopes confirmed. Penny label config (in-memory v1). Shipped Phase 1 sprint.',
    tags: ['Gmail', 'Right Panel', 'Actions'],
    icon: Mail,
    hue: 'sky',
    dependencies: [
      'Google OAuth token with gmail.readonly and gmail.compose scopes — ✓ done',
      'Google OAuth Setup flow complete (/admin/google-oauth) — ✓ done',
      'Right Panel (ContextBar) infrastructure stable — ✓ done',
      'Gemini API key valid for AI-drafted replies — ✓ done',
    ],
    roleImpact: [
      { role: 'Everyday User',    impact: 'Read surface — unanswered threads and follow-up reminders' },
      { role: 'Penny Power User', impact: 'Penny-drafted replies, thread summaries via /api/penny/ask; real send with confirmation' },
      { role: 'Admin',            impact: 'Penny label configuration (in-memory v1); escalation threshold config is Phase 2' },
    ],
    acceptanceCriteria: [
      '✓ Real inbox surfaced — 15 threads from live Gmail account via gmail.readonly',
      '✓ Penny can draft a follow-up reply before the user sends (Gemini 2.5 Flash)',
      '✓ Real send via POST /api/gmail/send — no email sent without explicit user confirmation',
      '✓ Thread read and subject/sender display in GmailCenter workspace',
      '✓ Zero-state handled gracefully when inbox is empty',
      'Escalation summary for threads with 3+ participants — Phase 2 enhancement',
    ],
  },
  {
  ...
  },
  {
    ...
    phase: 'Phase 2',
    owner: 'TBD',
    summary: 'Fully live. Google Sign-In via Clerk v6 (branded /sign-in, Google OAuth, role-gated routes) and Google Groups auto-tier assignment via Admin Directory API — service account DWD configured, real group membership drives tier on every login.',
    tags: ['Auth', 'Google', 'Access Control'],
    icon: LogIn,
    hue: 'rose',
    dependencies: [
      '✅ Clerk v6 Google Sign-In live — @clerk/react 6.9.1, branded /sign-in, signed-in/out gates',
      '✅ 3 Google Groups defined — trailosadmin, trailospennyadmin, trailosusers @transitiontrails.org',
      '✅ /api/auth/tier + /api/auth/groups-status endpoints built',
      '✅ GOOGLE_ADMIN_CREDENTIALS — service account with DWD + Directory API scope configured',
      '✅ GOOGLE_ADMIN_IMPERSONATE_EMAIL — Workspace admin impersonation wired',
    ],
    roleImpact: [
      { role: 'Everyday User',    impact: 'Google SSO live — tier auto-assigned from Google Group membership on every login' },
      { role: 'Penny Power User', impact: 'trailospennyadmin group membership grants Power tier capabilities automatically' },
      { role: 'Admin',            impact: 'trailosadmin group membership grants Admin tier — angela@, hugh@, technology@ confirmed live' },
      { role: 'Super Admin',      impact: 'Tier mapping driven by Workspace group membership — no code deploy needed to add/remove users' },
    ],
    acceptanceCriteria: [
      '✅ Login via Google SSO (no username/password form)',
      '✅ Unmapped users fall back to Everyday User role (not blocked)',
      '✅ Role-gated routes — Everyday User cannot reach admin tools via direct URL',
      '✅ Google Group membership queried in real time via Admin Directory API (member.readonly scope)',
      '✅ Role changes take effect on next login — add/remove user from Workspace group, no code deploy',
    ],
  },
  {
  ...
  },
  {
    ...
    phase: 'Phase 2',
    owner: 'Tech Lead',
    summary: 'Live at /penny/asset-library (Admin+ tab in Penny hub). Backed by Google Drive Shared Drive (TT Content → Penny Asset Library) — supportsAllDrives + includeItemsFromAllDrives active. Six Penny states map to Drive subfolders with real file counts and thumbnail grid (3-per-row, face-anchored). ElevenLabs/HeyGen API integrations remain Phase 2.',
    tags: ['Penny', 'Media', 'Assets'],
    icon: Image,
    hue: 'pink',
    dependencies: [
      '✓ GOOGLE_DRIVE_REFRESH_TOKEN — Drive OAuth active',
      '✓ GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET — OAuth credentials live',
      '✓ Penny state machine defined — 6 states map to Drive subfolders',
      '✓ GOOGLE_DRIVE_PENNY_FOLDER_ID — set and confirmed (Shared Drive: TT Content → Penny Asset Library)',
      '✓ Shared Drive support — supportsAllDrives + includeItemsFromAllDrives params active on all API calls',
      'ElevenLabs API key + Penny voice ID — Phase 2',
      'HeyGen API key + Penny avatar ID — Phase 2',
    ],
    roleImpact: [
    ...
    acceptanceCriteria: [
      '✓ Asset library UI at /penny/asset-library — Admin+ tab in Penny hub',
      '✓ Six Penny states each map to a named Drive subfolder (coaching, trail-talk, resume-review, interview-prep, confidence-builder, quest-debrief)',
      '✓ Asset cards show thumbnail (Drive thumbnailLink), mimeType badge, modified date, open-in-Drive link',
      '✓ Drive connection status badge — live/not connected with setup instructions',
      '✓ GOOGLE_DRIVE_PENNY_FOLDER_ID config strip — shows folder name, asset count, setup guide',
      '✓ Ungrouped assets section for root-level Drive files',
      'ElevenLabs voice playback works in-browser — Phase 2 (needs ElevenLabs API key)',
      'HeyGen avatar renders in Test Penny panel (/penny/test) — Phase 2 (needs HeyGen API key)',
    ],
  },
  {
