  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-4 space-y-4">
        ...
        {/* Header */}
        <div>
          <p className="text-[14px] font-bold  text-muted-foreground/50 mb-0.5">Administration — Audit</p>
          ...
          <p className="text-[14px] text-muted-foreground mt-0.5 max-w-2xl">
            Comprehensive record of every create/add action added across Trail OS. All use the shared <code className="text-[14px] bg-muted rounded px-1">CreatePanel</code> pattern — in-page, Draft status, consistent Cancel / Save Draft / Save &amp; View controls.
          </p>
        ...
        <div className="rounded-lg border border-border bg-white p-4 space-y-3">
          <p className="text-[14px] font-bold  text-muted-foreground/60">Shared Pattern — CreatePanel</p>
            ...
            {[
              ...
              <div key={p.label} className="rounded-lg border border-border bg-muted/20 px-3 py-2.5">
                <div className="flex items-start gap-1.5 mb-1">
                  ...
                  <p className="text-[14px] font-bold text-foreground leading-snug">{p.label}</p>
                </div>
                <p className="text-[14px] text-muted-foreground leading-snug">{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="rounded-xl border border-border overflow-hidden">
            ...
            {ADDED.map((entry, i) => {
              ...
              return (
                ...
                >
                  <div>
                    <p className="text-[14px] font-bold text-foreground leading-snug">{entry.workspace}</p>
                    <p className="text-[14px] text-muted-foreground leading-snug">{entry.section}</p>
                    <Link href={entry.path}>
                      <span className="text-[14px] text-primary/60 hover:text-primary transition-colors cursor-pointer">
                        {entry.path} →
                      </span>
                    </Link>
                  </div>
                  <div>
                    <span className={`inline-flex items-center gap-1 text-[14px] font-bold rounded-full border px-2 py-0.5 ${cfg.bg} ${cfg.cls}`}>
                      <StatusIcon className="w-2.5 h-2.5" />
                      {cfg.label}
                    </span>
                    <p className="text-[14px] font-bold text-foreground mt-1">{entry.objectType}</p>
                    <p className="text-[14px] text-muted-foreground">{entry.action}</p>
                  </div>
                  <p className="text-[14px] text-muted-foreground leading-snug">{entry.entryPoint}</p>
                  <p className="text-[14px] text-muted-foreground leading-snug">{entry.notes}</p>
                </div>
              );
            })}
          </div>
        ...
        <div>
          ...
          <div className="rounded-xl border border-border overflow-hidden">
            ...
            {READ_ONLY.map((entry, i) => (
              ...
              >
                <p className="text-[14px] font-semibold text-foreground">{entry.workspace}</p>
                <div className="flex items-center gap-1.5">
                  <MinusCircle className="w-3 h-3 text-muted-foreground/40 shrink-0" />
                  <p className="text-[14px] text-muted-foreground leading-snug">{entry.view}</p>
                </div>
                <p className="text-[14px] text-muted-foreground leading-relaxed">{entry.reason}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border/60 bg-muted/20 px-5 py-4">
          <p className="text-[14px] text-muted-foreground leading-relaxed">
            <strong>Prototype status:</strong> All create panels are wired to the in-page <code className="text-[14px] bg-muted rounded px-1">CreatePanel</code> component.
            New objects appear immediately in the UI but reset on page refresh until a live backend (Express/PostgreSQL) is wired in.
            When the API is connected, <code className="text-[14px] bg-muted rounded px-1">onSaveDraft</code> and <code className="text-[14px] bg-muted rounded px-1">onSaveAndView</code> callbacks
