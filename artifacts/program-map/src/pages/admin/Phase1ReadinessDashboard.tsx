...
const AREAS: ReadinessArea[] = [
  ...
  },
  {
    ...
    completed: [
      ...
      'GEMINI_API_KEY configured — format accepted (AQ. or AIza prefix), API reachable',
      ...
      'Live validation endpoints: /api/gemini/validate, /api/google/validate, /api/salesforce/validate',
    ],
    gaps: [
      'GEMINI_API_KEY returns INVALID_ARGUMENT — new AQ. format key needed from aistudio.google.com',
      ...
      'Agentforce API credentials not yet configured (Phase 2)',
      'Slack channels:read + groups:read scopes not yet added',
    ],
    blockers: [
      'Gemini key auth — generate new key at aistudio.google.com and add to Replit Secrets',
      'Google OAuth refresh tokens — run /admin/google-oauth flow to unblock Drive and Calendar sources',
    ],
    nextActions: [
      'Generate new GEMINI_API_KEY at aistudio.google.com and add to Replit Secrets',
      'Complete Google OAuth flow at /admin/google-oauth to get GOOGLE_DRIVE_REFRESH_TOKEN + GOOGLE_CALENDAR_REFRESH_TOKEN',
  ...
  },
  {
    ...
    nextActions: [
      ...
      'Link Google Drive folders per program in source registry once GOOGLE_DRIVE_REFRESH_TOKEN is obtained',
    ],
  },
  {
    id: 'penny',
...
const OVERALL: ReadinessArea = {
  ...
  gaps: [
    ...
    'Slack live (POC) — Penny delivery pipeline not yet wired end-to-end',
    'Google OAuth refresh tokens pending — blocks Drive, Calendar, and Knowledge sources',
    'Gemini live key needed — blocks Penny live responses',
    'Penny not yet wired to any live data source or API endpoint',
  ],
  blockers: [
    'Penny API wiring — wire first capability to GEMINI_API_KEY and Salesforce query endpoint',
    'Google OAuth refresh tokens — run /admin/google-oauth to unblock Drive, Calendar, and Knowledge sources',
  ],
  nextActions: [
    'Wire first Salesforce data query to health dashboard (REST API live at /api/salesforce/validate)',
    'Generate new GEMINI_API_KEY at aistudio.google.com and wire first Penny capability',
    'Complete Google OAuth flow at /admin/google-oauth (unblocks Drive + Calendar + Knowledge)',
