...
const AREAS: ReadinessArea[] = [
  ...
  },
  {
    ...
    completed: [
      ...
      'Access tier architecture live — 3 Google Groups mapped in accessTiers.ts; /api/auth/tier + /api/auth/groups-status endpoints built',
    ],
    gaps: [
      'Google Admin Directory API token not yet configured — group membership auto-tier awaits GOOGLE_ADMIN_CREDENTIALS secret; falls back to "everyday"',
    ...
    blockers: [],
    nextActions: [
      'Configure GOOGLE_ADMIN_CREDENTIALS (service account with Directory API scope) — unlocks Google Groups auto-tier assignment',
...
const OVERALL: ReadinessArea = {
  ...
  gaps: [
    'Google Groups auto-tier not yet live — GOOGLE_ADMIN_CREDENTIALS (Directory API service account) not yet configured; all users fall back to "everyday"',
  ...
  blockers: [],
  nextActions: [
    'Configure GOOGLE_ADMIN_CREDENTIALS service account with Directory API — last step to activate Google Groups auto-tier',
    'Wire Salesforce data query to health dashboard (REST API live at /api/salesforce/validate)',
