...
const SECTIONS: Section[] = [
  {
    ...
    standards: [
      ...
      },
      {
        ...
        rationale: 'Showing fewer things, using progressive disclosure, and splitting content across tabs achieves density without sacrificing legibility. Shrinking text does not make a screen feel organized — it makes it illegible and exhausting.',
  ...
  },
  {
    ...
    standards: [
      ...
      {
        id: 'typ-5',
        rule: 'Title Case only for proper program and trail names.',
        rationale: "Program names like \"Explorer's Trail\" and \"Career Launch Cohort 3\" are proper names and use Title Case. Interface labels, descriptions, section headers, tab names, and button text use sentence case.",
        doExample: '"Explorer\'s Trail" (program name), "Career Launch Cohort 3" (trail name). vs "Program overview" (interface label), "Health indicators" (section header).',
  ...
  },
  {
    ...
    standards: [
      {
        id: 'col-1',
        rule: 'Use token classes only in screen code — no raw hex values and no Tailwind framework color utilities.',
        rationale: 'Raw hex values and Tailwind color utilities (bg-emerald-100, text-sky-700) couple a screen to specific color values that cannot be updated globally. Token classes (bg-card, text-foreground, text-muted-foreground, border-border) adapt to theme changes automatically. For status, import STATUS_CLASSES from src/config/statusColors.',
      ...
      {
        id: 'col-2',
        rule: 'Four brand colors, each with a single intent.',
        rationale: 'Using a brand color outside its intent weakens the system and confuses users who have learned what each color means.',
      ...
      },
      {
        ...
        rationale: 'More than five status colors require users to memorise a legend. Five roles cover every operational state in Trail OS.',
      ...
      },
      {
        ...
        rationale: 'Amber (#F5A623, bg-accent) is the brand\'s primary CTA color. It signals "the most important action on this screen". Using it for status fills, category labels, or decoration dilutes that signal. Attention status uses amber text on a tint — not an amber fill.',
      ...
      {
        id: 'col-5',
        rule: 'Meaning is never carried by color alone. Every status keeps a text label and an icon.',
        rationale: 'Color-blind users must be able to read status without relying on hue. A colored dot on its own is not a status indicator — it is decoration.',
        ...
        dontExample: 'A standalone colored dot as the only status indicator, with no label or icon alongside it.',
      },
      {
        id: 'col-6',
        rule: 'Two background colors maximum per surface. No categorical color for programs, person types, or roles.',
        rationale: 'More than two background values on one surface creates visual noise and collapses the hierarchy. Categorical color-coding forces users to memorise a legend; label text carries the same distinction more clearly and without the overhead.',
        doExample: 'Trail Light page background + white card. A program is distinguished by its name and a status badge, not a hue.',
        dontExample: 'Six program cards each with a distinct color header. A person-type badge that is teal for "Staff" and amber for "Volunteer".',
      },
      {
        id: 'col-7',
        rule: 'Third-party brand marks keep their own colors and must never be converted.',
        rationale: 'Modifying a third-party logo or brand color violates trademark guidelines and misleads users about product origin.',
        doExample: 'The Gmail logo uses Google brand colors. The Salesforce logo uses Salesforce blue. These are left unchanged in any color sweep.',
        dontExample: 'Converting fill-rose-400 on a Gmail SVG icon to a Trail OS brand color during a color standardization pass.',
      },
      {
        id: 'col-8',
        rule: 'The functional red is not a brand color. It exists because an operations tool must distinguish a blocker from a warning.',
        rationale: 'The Transition Trails brand book has no red. An operations tool that cannot show a critical error is broken. The functional red (#A93F2F on #FBEAE6) is the only non-brand color permitted in Trail OS and is pending a brand book entry.',
  ...
  },
  {
    ...
    standards: [
      ...
      {
        id: 'int-2',
        rule: 'Cards lift 3 px on hover over approximately 160 ms with the card shadow. Buttons shift color on hover.',
      ...
      },
      {
        ...
        rationale: 'A press state that shrinks or bounces is disorienting in a dense data tool. Darkening the color or background on active state confirms the click without spatial movement.',
    ...
    standards: [
      {
        ...
        doExample: '/program lands on the Program Overview (health metrics, blueprint coverage, Penny coverage). The program list is a secondary tab.',
        dontExample: 'A split pane with a list on the left and an empty "select a program" placeholder on the right as the default landing state.',
      },
      {
        id: 'lay-2',
      ...
      },
      {
        ...
        doExample: 'HubShell renders underline tab bars automatically. The active tab has a bottom border in the primary color.',
  ...
  },
  {
    ...
    standards: [
      {
        id: 'vce-1',
        rule: 'Sentence case in all UI text. Title Case only for proper program and trail names.',
        ...
        doExample: '"Filter by tier", "Access tiers and auth", "Your programs this week".',
        dontExample: '"Filter By Tier", "Access Tiers And Auth", "Your Programs This Week".',
      },
      {
        id: 'vce-2',
        ...
        doExample: '"Your assigned programs are below." "We could not reach Salesforce — check your credentials." "3 roles need an owner assigned."',
  ...
  },
  {
    ...
    standards: [
      ...
      },
      {
        ...
        doExample: '"Your programs" (Everyday) vs "Program management" (Admin). One visible tab means no tab bar is shown.',
      ...
      },
      {
        ...
        doExample: 'Wrap integration notice banners in isAdminOrAbove checks. The Everyday view of Operations shows program health, not integration health.',
      ...
      },
      {
        ...
        rationale: 'These tools enrich the experience behind the scenes. Everyday Users benefit from their output — relevant context in the right panel, enriched program data — without needing to know the mechanism.',
...
const CHANGES: Change[] = [
  ...
  },
  {
    ...
    reason: 'Amber is the brand CTA color. Over-use dilutes the signal that a primary action is available on this screen.',
  },
  {
...
// ── Color map for section card rendering ─────────────────────────────────────
