      ...
      <div className="p-6 max-w-5xl space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {['all', 'high', 'medium', 'low'].map(sev => (
            ...
            >
              {sev === 'all' ? 'All Issues' : `${sev.charAt(0).toUpperCase() + sev.slice(1)} (${counts[sev as HealthSeverity]})`}
            </button>
          ))}
        </div>
          ...
          {filtered.map(issue => {
            ...
            return (
              <button
                ...
                className="rounded-xl border border-border bg-white p-4 text-left hover:border-red-200 hover:shadow-sm transition-all"
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex items-center gap-2">
                    <AlertTriangle className={`w-4 h-4 shrink-0 ${issue.severity === 'high' ? 'text-red-500' : issue.severity === 'medium' ? 'text-orange-500' : 'text-amber-500'}`} />
                    <div>
                ...
                {((issue.affectedItems as string[]) || []).length > 0 && (
                  <div className="mt-1 space-y-0.5">
                    {(issue.affectedItems as string[]).map((item, i) => (
                      <p key={i} className="text-[11px] text-red-700">• {item}</p>
                    ))}
                  </div>
                )}
