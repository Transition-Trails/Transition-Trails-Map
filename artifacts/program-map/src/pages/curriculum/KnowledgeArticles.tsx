    ...
    <ScrollArea className="h-full">
      <div className="p-6 max-w-5xl space-y-5">
        ...
        <div className="flex gap-2 flex-wrap">
          {articleTypes.map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} className={`text-[11px] font-semibold rounded-full px-3 py-1.5 border transition-colors capitalize ${typeFilter === t ? 'bg-indigo-600 text-white border-indigo-600' : 'border-border text-muted-foreground hover:border-indigo-400'}`}>
              {t === 'all' ? 'All Types' : t}
            </button>
          ))}
        </div>
        <div className="grid gap-3">
          {filtered.map(article => {
            ...
            return (
              <button key={article.id} onClick={() => setSelectedItem({ type: 'curriculumItem', id: article.id, data: article })} className="rounded-xl border border-border bg-white p-4 text-left hover:border-indigo-200 hover:shadow-sm transition-all">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] font-medium text-indigo-700 border border-indigo-200 bg-indigo-50 rounded-full px-1.5 py-0.5">{article.articleType as string}</span>
