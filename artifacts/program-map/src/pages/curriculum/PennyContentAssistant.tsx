...
import { Sparkles, ArrowRight, Layers, BookOpen, CheckCircle2, MessageCircle, Star } from 'lucide-react';
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="shrink-0 border-b border-border bg-background px-5 pt-4 pb-3">
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-[14px] font-semibold text-foreground">Content Workshop</h1>
          <span className="text-muted-foreground/30 text-[12px] hidden sm:inline">·</span>
          <span className="text-[12px] text-muted-foreground">
            <span className="font-bold text-foreground">{pennyContentActions.length}</span> Actions
          </span>
          <span className="text-muted-foreground/30 text-[12px]">·</span>
          <span className="text-[12px] text-muted-foreground">
            <span className="font-bold text-foreground">{TYPE_CONFIG.length}</span> Object Types
          </span>
        </div>
      </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div className="lg:col-span-2 space-y-3">
              <p className="text-[14px] font-bold text-muted-foreground/60">1 — Select Object Type</p>
              <div className="flex flex-wrap gap-1.5">
                {TYPE_CONFIG.map(tc => (
                  <button
                    key={tc.id}
                    onClick={() => { setSelectedType(tc.id); setSelectedObjectId(tc.data[0]?.id || ''); }}
                    className={`text-[14px] font-semibold rounded-full px-3 py-1 border transition-colors ${selectedType === tc.id ? 'bg-secondary text-white border-secondary' : 'border-border text-muted-foreground hover:border-secondary/40'}`}
                  >
                    {tc.label}
                  </button>
                ))}
              ...
              <p className="text-[14px] font-bold text-muted-foreground/60 mt-3">2 — Select Object</p>
                ...
                {currentObjects.map(obj => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isMod21 && <p className="text-[14px] text-secondary font-semibold mt-0.5">Standards reference example</p>}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="lg:col-span-3 space-y-3">
              <div className="flex items-center gap-2">
                <p className="text-[14px] font-bold text-muted-foreground/60">3 — Available Actions</p>
                {selectedObject && (
                  <span className="text-[14px] text-secondary font-medium border border-secondary/20 bg-secondary/5 rounded-full px-2 py-0.5">
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          ...
                          <p className="text-[14px] text-muted-foreground">{action.contextSentence}</p>
                          <div className="flex flex-wrap gap-1 mt-2">
                            {action.generates.slice(0, 3).map(g => (
                              <span key={g.label} className="text-[14px] text-muted-foreground border border-border rounded-full px-2 py-0.5">{g.label}</span>
                            ))}
                            {action.generates.length > 3 && (
                              <span className="text-[14px] text-muted-foreground border border-border rounded-full px-2 py-0.5">+{action.generates.length - 3} more</span>
                            )}
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          <span className="text-[14px] text-muted-foreground">{action.estimatedTime}</span>
              ...
              {otherActions.length > 0 && (
                <div className="mt-2">
                  <p className="text-[14px] text-muted-foreground/60 font-semibold mb-2">Also available (select a different object type to use)</p>
                  <div className="grid grid-cols-2 gap-1.5">
                    {otherActions.map(action => (
                      ...
                      >
                        <p className="text-[14px] font-semibold text-foreground/70">{action.shortName}</p>
                        <p className="text-[14px] text-muted-foreground/60">{action.category}</p>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="flex items-start justify-between gap-4 mb-3">
              <div>
                ...
                <p className="text-[14px] text-muted-foreground">
                  Full output from running "Generate Module Outline" + 6 supporting actions on Module 2.1. Includes learning objectives, lesson structure, assessment alignment, knowledge articles, coach notes, Slack activities, and calendar events.
                </p>
              </div>
              <button
                onClick={() => setLocation('/curriculum/generated-outputs')}
                className="flex items-center gap-1.5 text-[14px] font-semibold text-primary border border-primary/20 bg-white rounded-full px-3 py-1.5 hover:bg-primary/10 transition-colors whitespace-nowrap shrink-0"
              >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                ...
                <div key={item.label} className="rounded-lg border border-primary/20 bg-white px-3 py-2">
                  <p className="text-[14px] font-semibold text-primary">{item.label}</p>
