      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="rounded-xl border border-primary/20 bg-primary/5 px-5 py-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-[13px] font-bold text-foreground">We are currently in Phase 1</p>
          </div>
          <div className="space-y-2">
            {CURRENT_PHASES.map(p => (
              <div key={p.phase} className="flex items-center gap-3">
                ...
                <div>
                  <span className={`text-[12px] font-semibold ${p.active ? 'text-primary' : 'text-muted-foreground'}`}>{p.phase}: </span>
                  <span className={`text-[12px] ${p.active ? 'text-foreground' : 'text-muted-foreground'}`}>{p.label}</span>
                  {p.active && <span className="ml-2 inline-flex text-[10px] font-semibold border border-primary/20 bg-primary/10 text-primary rounded-full px-2 py-0.5">Current</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Future capabilities */}
        <section>
          <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground/60 mb-4">Future Content Request Types</h2>
          <div className="space-y-2">
            {FUTURE_CAPABILITIES.map(cap => (
              <div key={cap.label} className="flex items-start gap-3 rounded-xl border border-border bg-white p-4">
                ...
                <div>
                  <p className="text-[13px] font-semibold text-foreground">{cap.label}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">{cap.desc}</p>
                </div>
                <span className="ml-auto inline-flex flex-shrink-0 text-[10px] font-medium text-muted-foreground border border-border bg-muted/40 rounded-full px-2 py-0.5">Future</span>
              </div>
            ))}
          </div>
        </section>
