...
const SECTION_TABS = [
  ...
  { id: 'lessons',      label: 'Lesson Structure',      icon: '📚' },
  { id: 'assessment',   label: 'Assessment Alignment',  icon: '📝' },
  { id: 'articles',     label: 'Knowledge Articles',    icon: '📖' },
  { id: 'coach',        label: 'Coach Notes',           icon: '👩‍🏫' },
  { id: 'reflection',   label: 'Reflection Prompts',    icon: '💭' },
  { id: 'slack',        label: 'Slack Activities',      icon: '💬' },
    ...
    <ScrollArea className="h-full">
      <div className="p-6 max-w-5xl space-y-5">
        ...
        <div>
          ...
          <h1 className="text-3xl font-serif font-bold text-foreground">Generated Outputs</h1>
          <p className="text-[13px] text-muted-foreground mt-1">
            Sample content generated by Penny for <strong>{output.moduleName}</strong> — {output.sprint}.
            This represents the complete output from running all 7 applicable content generation actions.
            All content is prototype-quality — ready for staff review and integration.
          </p>
        </div>
        ...
        <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-4">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[10px] font-bold text-primary border border-primary/20 bg-white rounded-full px-2 py-0.5">★ Standards Reference</span>
            <span className="text-[10px] text-muted-foreground">{output.generatedAt}</span>
          </div>
          <p className="text-[15px] font-bold text-foreground mb-1">{output.moduleName}</p>
            ...
            {output.generatedBy.map(actionId => {
              ...
              return action ? (
                <button
                  ...
                  className="text-[10px] font-medium border border-secondary/20 bg-secondary/10 text-secondary rounded-full px-2 py-0.5 hover:bg-secondary/20 transition-colors"
                >
                  {action.shortName} →
                </button>
              ) : null;
        ...
        <div className="flex flex-wrap gap-1.5">
          {SECTION_TABS.map(tab => (
            <button
              ...
              className={`text-[11px] font-semibold rounded-full px-3 py-1.5 border transition-colors ${activeSection === tab.id ? 'bg-secondary text-white border-secondary' : 'border-border text-muted-foreground hover:border-secondary/40'}`}
            >
              {tab.icon} {tab.label}
            </button>
          ))}
          ...
          {activeSection === 'objectives' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={() => showAction('generate-module-outline')} className="ml-auto text-[10px] text-secondary font-medium hover:underline flex items-center gap-1">Generate Module Outline <ArrowRight className="w-3 h-3" /></button>
              </div>
              <p className="text-[11px] text-muted-foreground italic mb-3">All objectives are measurable and verb-first, aligned to Bloom's Taxonomy. Each maps to at least one lesson and one assessment question area.</p>
                ...
                {output.objectives.map((obj, i) => (
                  <div key={i} className="rounded-lg border border-border bg-primary/5 p-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] font-bold text-primary/60 border border-primary/20 rounded-full px-1.5 py-0.5">{obj.level}</span>
                      <span className="text-[10px] font-medium text-primary/70">Verb: {obj.verb}</span>
                    </div>
                    <p className="text-[13px] font-semibold text-foreground">{obj.text}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeSection === 'lessons' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={() => showAction('create-lesson')} className="ml-auto text-[10px] text-secondary font-medium hover:underline flex items-center gap-1">Create Lesson <ArrowRight className="w-3 h-3" /></button>
              </div>
              <div className="space-y-4">
                {output.lessonStructure.map((lesson, i) => (
                  <div key={i} className="rounded-xl border border-amber-200 bg-amber-50/30 p-4">
                    ...
                    <p className="text-[11px] text-muted-foreground mb-3"><strong>Objective:</strong> {lesson.objective}</p>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg border border-rose-200 bg-rose-50/30 p-4 mb-4">
                ...
                <div className="flex flex-wrap gap-3 mt-2">
                  ...
                  <div><p className="text-[14px] font-bold text-foreground">{output.assessmentAlignment.passingScore}%</p><p className="text-[10px] text-muted-foreground">Passing Score</p></div>
                  <div><p className="text-[12px] font-bold text-foreground">{output.assessmentAlignment.type}</p><p className="text-[10px] text-muted-foreground">Type</p></div>
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/60">Question Areas (mapped to objectives)</p>
                {output.assessmentAlignment.questionAreas.map((area, i) => (
                  <div key={i} className="flex items-center gap-3 rounded-lg border border-border bg-white px-4 py-2.5">
                    <span className="text-[13px] font-bold text-foreground w-6 text-center">{area.count}</span>
                    <div>
                      <p className="text-[12px] font-semibold text-foreground">{area.area}</p>
                      <p className="text-[10px] text-muted-foreground">{area.objectives.join('; ')}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="space-y-2 mt-3">
                <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/60">Sample Questions</p>
                {output.assessmentAlignment.sampleQuestions.map((q, i) => (
                  <div key={i} className="rounded-lg border border-border bg-slate-50 px-4 py-3">
                    <p className="text-[12px] text-foreground italic">"{q}"</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeSection === 'articles' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={() => showAction('create-knowledge-article')} className="ml-auto text-[10px] text-secondary font-medium hover:underline flex items-center gap-1">Create KB Article <ArrowRight className="w-3 h-3" /></button>
              </div>
              <div className="space-y-4">
          ...
          {activeSection === 'coach' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={() => showAction('create-coach-notes')} className="ml-auto text-[10px] text-secondary font-medium hover:underline flex items-center gap-1">Create Coach Notes <ArrowRight className="w-3 h-3" /></button>
              </div>
              <div className="rounded-lg border border-border bg-slate-50 p-4 mb-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60 mb-1">Facilitator Intent Summary</p>
                <p className="text-[12px] text-foreground leading-relaxed">{output.coachNotes.intentSummary}</p>
              </div>
              <div className="space-y-2">
                <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/60">Common Misconceptions</p>
                {output.coachNotes.commonMisconceptions.map((item, i) => (
                  <div key={i} className="rounded-lg border border-border bg-white p-3">
                    <p className="text-[12px] font-semibold text-foreground mb-1">❌ "{item.misconception}"</p>
                    <p className="text-[11px] text-muted-foreground"><strong>Coach response:</strong> {item.response}</p>
                  </div>
                ))}
              </div>
              <div className="space-y-1 mt-3">
                <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/60">Conversation Starters</p>
                {output.coachNotes.conversationStarters.map((q, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <Users className="w-3 h-3 text-muted-foreground mt-0.5 shrink-0" />
                    <p className="text-[11px] text-foreground/80 italic">"{q}"</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeSection === 'reflection' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={() => showAction('create-reflection-prompt')} className="ml-auto text-[10px] text-secondary font-medium hover:underline flex items-center gap-1">Create Reflection Prompt <ArrowRight className="w-3 h-3" /></button>
              </div>
              <div className="space-y-4">
          ...
          {activeSection === 'slack' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={() => showAction('create-slack-activity')} className="ml-auto text-[10px] text-secondary font-medium hover:underline flex items-center gap-1">Create Slack Activity <ArrowRight className="w-3 h-3" /></button>
              </div>
              <div className="space-y-4">
          ...
          {activeSection === 'calendar' && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={() => showAction('create-calendar-reminder')} className="ml-auto text-[10px] text-secondary font-medium hover:underline flex items-center gap-1">Create Calendar Reminder <ArrowRight className="w-3 h-3" /></button>
              </div>
              <div className="space-y-4">
