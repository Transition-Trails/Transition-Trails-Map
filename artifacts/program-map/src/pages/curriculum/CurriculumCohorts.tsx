                ...
                <div className="flex flex-wrap gap-4 mb-2">
                  ...
                  {atRisk > 0 && (
                    <div>
                      ...
                      <p className="text-[10px] text-[#CC8400]">At Risk</p>
                    </div>
                  )}
                  {!!(cohort.completionRate) && (
                    <div><p className="text-[14px] font-bold text-[#2F6B3F]">{cohort.completionRate as string}</p><p className="text-[10px] text-muted-foreground">Completion</p></div>
                  )}
                </div>
                {!!(cohort.currentSprint) && (
                  <p className="text-[11px] text-primary font-medium">Currently: {cohort.currentSprint as string}</p>
                )}
                {cohort.notes && <p className="text-[11px] text-[#CC8400] bg-[#FFF3E0] border border-[#FFF3E0] rounded px-2 py-1 mt-2">{cohort.notes}</p>}
              </button>
            );
          })}
        </div>
