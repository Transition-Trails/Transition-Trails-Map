  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <p className={`text-[12px] font-semibold leading-tight ${selected ? 'text-primary' : 'text-foreground'}`}>
      ...
      <div className="flex items-center justify-between gap-2">
        <span className={`text-[9px] font-semibold border rounded-full px-1.5 py-0.5 ${statusCfg.cls}`}>{statusCfg.label}</span>
        {total > 0 && (
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-5 border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <span className={`text-[10px] font-semibold border rounded-full px-2 py-0.5 ${st.pill}`}>{st.label}</span>
      </div>
    </div>
  );
}
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">Salesforce Course__c · {total} modules</p>
            </div>
          </div>
          <span className={`text-[10px] font-semibold border rounded-full px-2 py-0.5 shrink-0 ${statusCfg.cls}`}>
            {statusCfg.label}
          </span>
        </div>
          ...
          <ScrollArea className="h-full">
            <div className="py-2">
              ...
              <div className="flex items-center gap-3 px-5 pb-2 border-b border-border/40">
                ...
                <p className="flex-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">Module</p>
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 shrink-0">Status</p>
              </div>
            ...
            <div className="flex gap-4 text-right">
              ...
              {data.fromCache && (
                <div className="flex items-end pb-0.5">
                  <span className="text-[10px] text-muted-foreground/50">cached</span>
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex-1 overflow-hidden bg-muted/10">
          {selected ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center gap-3 text-muted-foreground">
              ...
              <p className="text-[13px]">Select a course to view its modules</p>
            </div>
          )}
        </div>
      </div>
