...
export default function ProgramOverview() {
  const [, setLocation] = useLocation();
  const { programs, setSelectedItem, setAskPennyOpen, setPendingPennyQuery } = useAppContext();
  ...
  const pennyQ = `Programs overview:\n- Total programs: ${programs.length}\n- Confirmed: ${stats.confirmed}, Needs Review: ${stats.needsReview}, Draft: ${stats.draft}\n- ${TERMS.aiAssistant} Active: ${stats.pennyActive} programs\n- Without ${TERMS.aiAssistant}: ${stats.withoutPenny}\n- LMS: ${lmsSummary.total} courses, ${lmsSummary.completedModules}/${lmsSummary.totalModules} modules complete\n\nWhat are the 2–3 highest-impact actions to improve program governance and ${TERMS.aiAssistant} coverage?`;
  ...
  return (
    ...
    {/* ── Rail: program health summary ────────────────────────────────────── */}
    <div className="w-[272px] shrink-0 border-l border-border bg-muted/10 overflow-y-auto">
      <div className="p-4 space-y-4">

        {/* Program health */}
        <div className="rounded-lg border border-border bg-white p-3">
          <p className="text-[12px] font-bold text-muted-foreground/50 mb-2 uppercase tracking-wide">Program Health</p>
          <div className="flex items-end gap-2 mb-3">
            <span className="text-3xl font-bold leading-none text-foreground">{programs.filter(p => p.confidence !== 'deprecated').length}</span>
            <span className="text-[12px] text-muted-foreground mb-0.5">programs</span>
          </div>
          <div className="space-y-1.5">
            {[
              { label: 'Confirmed',    count: stats.confirmed,   bar: 'bg-[#2F6B3F]', text: 'text-[#2F6B3F]' },
              { label: 'Needs Review', count: stats.needsReview, bar: 'bg-[#CC8400]', text: 'text-[#CC8400]'  },
              { label: 'Draft',        count: stats.draft,       bar: 'bg-slate-300', text: 'text-slate-500'  },
            ].map(row => (
              <div key={row.label}>
                <div className="flex items-center justify-between mb-0.5">
                  <span className="text-[12px] text-muted-foreground">{row.label}</span>
                  <span className={`text-[12px] font-bold ${row.text}`}>{row.count}</span>
                </div>
                <div className="h-1 bg-muted rounded-full">
                  <div
                    className={`h-1 rounded-full ${row.bar}`}
                    style={{ width: `${programs.length ? Math.round((row.count / programs.length) * 100) : 0}%` }}
                  />
                </div>
              </div>
        ...
        {/* Penny coverage */}
        <div className="rounded-lg border border-border bg-white p-3">
          <p className="text-[12px] font-bold text-muted-foreground/50 mb-2 uppercase tracking-wide">{TERMS.aiAssistant} Coverage</p>
          <div className="flex items-end gap-3 mb-2">
            <div>
              <span className="text-xl font-bold text-primary">{stats.pennyActive}</span>
              <p className="text-[11px] text-muted-foreground">active</p>
            </div>
            {stats.withoutPenny > 0 && (
              <div>
                <span className="text-xl font-bold text-[#CC8400]">{stats.withoutPenny}</span>
                <p className="text-[11px] text-muted-foreground">unmapped</p>
              </div>
            )}
          </div>
          {lmsSummary.totalModules > 0 && (
            <div>
              ...
              <div className="h-1 bg-muted rounded-full">
                <div
                  className="h-1 rounded-full bg-[#2F6B3F]"
                  style={{ width: `${Math.round((lmsSummary.completedModules / lmsSummary.totalModules) * 100)}%` }}
                />
              </div>
            </div>
        ...
        {stats.needsReview > 0 && (
          <div className="rounded-lg border border-[#FFD08A] bg-[#FFF3E0]/60 p-3">
            <p className="text-[12px] font-bold text-[#CC8400] mb-1">
              {stats.needsReview} need{stats.needsReview === 1 ? 's' : ''} review
            </p>
            <p className="text-[11px] text-muted-foreground/70 leading-relaxed">
              Blueprint migration pending for {stats.needsReview} program{stats.needsReview !== 1 ? 's' : ''}.
            </p>
            <button
              onClick={() => setLocation('/program/blueprint')}
        ...
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-primary/60">Improve program governance</p>
          </div>
        </button>
        ...
        <div className="space-y-1">
          ...
          {[
            { label: 'Programs',         path: '/program/config' },
            { label: 'Blueprint Canvas', path: '/program/blueprint' },
            { label: 'Courses & Modules',path: '/program/courses' },
          ].map(link => (
            <button
              key={link.path}
