...
function buildCatalog(): SecretEntry[] {
  const entries: SecretSpec[] = [
    ...
    // ── Salesforce ─────────────────────────────────────────────────────────
    { id: "sf-webhook-secret",    primary: "SALESFORCE_WEBHOOK_SECRET",  alts: ["SF_WEBHOOK_SECRET"],         integration: "Salesforce", category: "Webhook Secret", required: true,  purpose: "Verifies incoming Salesforce event notifications and webhook payloads.", nextFix: "Generate and set in Salesforce Connected App or outbound message configuration.", fmtCheck: webhookSecretFormat },
    { id: "sf-client-id",         primary: "SALESFORCE_CLIENT_ID",       alts: ["SF_CLIENT_ID", "SFDC_CLIENT_ID"], integration: "Salesforce", category: "Connected App", required: false, purpose: "Salesforce Connected App client ID for OAuth 2.0 API access.", nextFix: "Salesforce Setup → App Manager → Connected App → Consumer Key." },
    { id: "sf-client-secret",     primary: "SALESFORCE_CLIENT_SECRET",   alts: ["SF_CLIENT_SECRET", "SFDC_CLIENT_SECRET"], integration: "Salesforce", category: "Connected App", required: false, purpose: "Salesforce Connected App client secret paired with SALESFORCE_CLIENT_ID.", nextFix: "Salesforce Setup → App Manager → Connected App → Consumer Secret." },
    { id: "sf-instance-url",      primary: "SALESFORCE_INSTANCE_URL",    alts: ["SF_INSTANCE_URL", "SFDC_INSTANCE_URL"], integration: "Salesforce", category: "Instance URL", required: false, purpose: "Your Salesforce org URL — required for all REST and Bulk API calls.", nextFix: "Your org URL: https://yourorg.my.salesforce.com." },
