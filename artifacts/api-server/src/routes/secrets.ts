...
function buildCatalog(): SecretEntry[] {
  const entries: SecretSpec[] = [
    ...
    // ── Gemini / Penny AI ──────────────────────────────────────────────────
    { id: "gemini-api-key",       primary: "GEMINI_API_KEY",            alts: ["GOOGLE_AI_API_KEY", "GOOGLE_AI_KEY", "PALM_API_KEY", "VERTEX_AI_API_KEY"], integration: "Gemini / Penny AI", category: "API Key", required: true,  purpose: "Powers Penny AI for learners (POST /api/penny/ask with learner session), quest submission feedback (POST /api/learner/quest/submit), and learner chat (POST /api/learner/penny/ask) — all on Gemini 2.5 Flash. Staff Penny uses Claude (ANTHROPIC_API_KEY).", nextFix: "Google AI Studio (aistudio.google.com) → Get API Key. New keys start with AQ. (secure auth key format); legacy keys start with AIza. Both formats are accepted.", fmtCheck: geminiKeyFormat },

    // ── Anthropic / Claude (staff Penny) ─────────────────────────────────
    { id: "anthropic-api-key",    primary: "ANTHROPIC_API_KEY",         alts: ["CLAUDE_API_KEY"], integration: "Anthropic / Claude", category: "API Key", required: true, purpose: "Powers Penny AI for staff (POST /api/penny/ask with internal session) via Claude Sonnet. Without this key, all staff Penny requests return 503. Gemini is NOT used as a fallback — the failure is explicit so outages are visible in logs immediately.", nextFix: "console.anthropic.com → API Keys → Create Key. Keys start with sk-ant-." },
