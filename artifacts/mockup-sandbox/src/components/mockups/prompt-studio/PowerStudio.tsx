...
interface PennyAsset { kind: "image" | "audio" | "video"; label: string; status: AssetStatus; hint: string; }

const PENNY_CONTENT = {
  folder: "Penny Content / Assets",
  connected: true,
  assets: [
    { kind: "image" as const, label: "penny-avatar.png",       status: "available"      as AssetStatus, hint: "Chat bubbles & cards" },
    { kind: "audio" as const, label: "penny-voice-intro.mp3",  status: "available"      as AssetStatus, hint: "Session open audio" },
    { kind: "video" as const, label: "penny-welcome.mp4",      status: "not-configured" as AssetStatus, hint: "Add to Drive to activate" },
  ],
};
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <div className="rounded-xl border border-[#D9EAE0] bg-[#F0F5F2] overflow-hidden">
          <div className="flex items-center gap-2.5 px-3 py-2 border-b border-[#D9EAE0]">
            {/* Avatar */}
            <div className="w-8 h-8 rounded-full bg-[#1B3A2D] flex items-center justify-center text-[12px] font-bold text-white ring-2 ring-white shadow-sm shrink-0">
              P
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="text-[12px] font-semibold text-[#1B3A2D]">Penny's content</span>
              ...
              <p className="text-[10px] text-[#64748B] truncate flex items-center gap-1">
                <FolderOpen className="w-2.5 h-2.5 shrink-0" />{PENNY_CONTENT.folder}
              </p>
            </div>
            <button className="text-[#64748B] hover:text-[#1B3A2D] transition-colors" title="Open Drive folder">
            ...
            {PENNY_CONTENT.assets.map(a => {
              ...
              return (
                <div key={a.kind} className="flex items-center gap-2.5 px-3 py-2">
                  <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${avail ? "bg-[#1B3A2D]" : "bg-[#E2E8F0]"}`}>
                    <Icon className={`w-3 h-3 ${avail ? "text-white" : "text-[#94A3B8]"}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-[11px] font-medium truncate ${avail ? "text-[#1E293B]" : "text-[#94A3B8]"}`}>{a.label}</p>
                    <p className="text-[10px] text-[#94A3B8]">{a.hint}</p>
                  </div>
                  <span className={`text-[9px] font-bold rounded-full px-1.5 py-0.5 shrink-0 ${
                    avail
                      ? "bg-[#C8E6D0] text-[#2F6B3F] border border-[#9FC3AE]"
                      : "bg-[#F1F5F9] text-[#94A3B8] border border-[#E2E8F0]"
                  }`}>{avail ? "Ready" : "Not set"}</span>
                </div>
              );
            })}
          </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-xl border border-[#E2E8F0] p-3.5">
            <div className="flex items-center gap-2 mb-2.5 pb-2 border-b border-[#F1F5F9]">
              ...
              <div className="w-6 h-6 rounded-full bg-[#1B3A2D] flex items-center justify-center text-[10px] font-bold text-white ring-1 ring-white shadow-sm">
                P
              </div>
